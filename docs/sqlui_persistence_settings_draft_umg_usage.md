...
- [`sqlui_smoke_test.md`](sqlui_smoke_test.md) lists the local smoke command that validates the draft model, apply preview, apply-preview display rows, validation display rows, presenter, and widget shell.
...
The persistence settings draft validation UMG foundation is complete as a binding scaffold. It includes the #105 settings editing/reset UX plan, the #106 SQLUICore draft model and validation result, the #107 SQLUICore display rows and summary, the #108 optional SQLUISamples presenter/adapter, the #109 optional C++ UMG widget shell, this #110 usage guide, the #111 final checkpoint, the SQLUICore dry-run apply-intent preview, UI-safe apply-preview display rows/summary, and `-UsePersistenceSettingsDraftProbe` non-asset smoke coverage for the draft model, apply preview, apply-preview display rows, validation display rows, presenter/adapter, and widget-shell contract.
...
The apply-intent preview and apply-preview display rows are report data only. They may use "would change" and "not applied" messaging, but they do not save settings, write config, initialize providers/repositories, create directories or database files, open databases for writing, run migrations, copy seeds, reset databases, or delete files.
