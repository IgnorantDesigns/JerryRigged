...
`USQLUISamplePersistenceStatusPresenter` is the first optional SQLUISamples sample/dev surface over those display rows. It stores the rows and stable formatted text lines for simple sample UI, Blueprint, or commandlet consumption. It is not wired into default startup, maps, or config, and it does not duplicate SQLUICore probing, edit settings, initialize providers/repositories, create databases, run migrations, copy seeds, reset databases, or delete files.
...
- Persistence status sample surface probe: SQLUISamples exercises `USQLUISamplePersistenceStatusPresenter`, verifies default `InMemory` rows and formatted lines are presented without initializing a provider or repository, verifies an explicit missing SQLite path is shown gracefully without creating a DB, verifies a smoke-owned sidecar is not deleted by refresh, and cleans up under `Saved/SQLUI/SmokeTests/PersistenceStatusSampleSurface`.
