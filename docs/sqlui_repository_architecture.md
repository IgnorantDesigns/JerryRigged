...
`USQLUISamplePersistenceStatusPresenter` is the first optional SQLUISamples sample/dev surface over those display rows. It stores the rows and stable formatted text lines for simple sample UI, Blueprint, or commandlet consumption, and exposes an explicit caller-invoked refresh result that re-queries the same SQLUICore status/display surfaces. It is not wired into default startup, maps, timers, tick, polling, or config, and it does not duplicate SQLUICore probing, edit settings, initialize providers/repositories, create databases, run migrations, copy seeds, reset databases, or delete files.
...
- Persistence status sample surface probe: SQLUISamples exercises `USQLUISamplePersistenceStatusPresenter`, verifies explicit and repeated refresh of default `InMemory` rows and formatted lines without initializing a provider or repository, verifies repeated refresh is deterministic and does not create a DB, verifies an explicit missing SQLite path is shown gracefully without creating a DB, verifies a smoke-owned sidecar is not deleted by refresh, and cleans up under `Saved/SQLUI/SmokeTests/PersistenceStatusSampleSurface`.
...
4. Implement the first persistence settings widget/panel slice from [`sqlui_persistence_settings_ux_design.md`](sqlui_persistence_settings_ux_design.md), binding to the read-only status snapshot and using the database management helper for any later status/reset flows instead of widget-owned file deletion.
