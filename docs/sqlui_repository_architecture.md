...
`FSQLUIPersistenceSettingsApplyConfigTargetPolicy` is the non-mutating resolver layer above that scaffold. It makes target selection explicit without enabling production Apply: default/runtime resolves unavailable and non-writable, explicit safe smoke-owned targets resolve write-capable for smoke/test coverage only, and the future project/user config target resolves unavailable/not implemented until a later PR chooses and validates a real target. `RequestPersistenceSettingsApply` remains the existing unavailable/non-mutating apply entrypoint and still keeps every side-effect flag false.

The #132 checkpoint proves target isolation only. It does not make `RequestPersistenceSettingsApply` a production Apply path and does not add settings controls or lifecycle ownership to SQLUISamples. The smoke-owned writer proves that SQLUICore can validate a draft before serialization, refuse invalid drafts without mutating the smoke target, distinguish explicit smoke/test targets from default/runtime targets, keep repo `Config` and `Saved/Config` unchanged, and clean up the temporary artifact. Any future production/user config-write path must choose a safe real target explicitly, keep writes narrow, keep `InMemory` as the default, keep SQLite and provider auto-init opt-in, avoid provider/repository initialization and DB work, and include config diff/snapshot coverage before UI controls are added.
