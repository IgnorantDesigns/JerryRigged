...
The SQLite repository proof now sits behind the same repository shape for `ListLayouts`, `LoadLayout`, writable `SaveLayout`, soft-delete `RemoveLayout`, and destructive scoped `ClearLayouts`, and a combined full lifecycle smoke path exercises those currently supported operations in one workflow. SQLite database work is separated into a non-UObject worker-safe helper. Callback-style `LoadLayout` and `SaveLayout` can opt into async execution; direct methods and the remaining repository operations stay synchronous for now. SQLite is factory-selectable only when explicitly requested and configured. The factory passes settings only; it does not run migrations or create database files. The SQLite repository can opt into schema initialization when both `bInitializeSchemaIfMissing` and, for missing database files, `bCreateDatabaseIfMissing` are enabled. Already-initialized databases are handled idempotently, complete schemas with missing initial migration rows are recorded non-destructively, partial schemas fail clearly with missing-object errors, and read-only mode rejects writes before schema initialization can create files. Full SQLite persistence remains future work. Callers should eventually be able to request, save, list, remove, and clear layouts without knowing whether the backing store is in memory, JSON files, or SQLite.
...
SQLite persistence is still incomplete. The first opt-in schema initialization slice and targeted schema-init hardening smoke coverage exist, but production migration versioning and upgrade paths, full async database execution, shutdown behavior, and packaged-build validation should happen in later implementation work.
