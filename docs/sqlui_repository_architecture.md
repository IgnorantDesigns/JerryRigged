...
`USQLUIPersistenceSettingsDraftLibrary` is the first non-mutating SQLUICore settings draft helper for future settings panels. It builds current/default draft data from runtime settings policy, represents pending runtime config and provider auto-init values, validates backend/path/provider-auto-init choices, can reset a draft value back to current values, builds dry-run apply-intent previews through `FSQLUIPersistenceSettingsApplyPreviewResult`, exposes non-executing apply/cancel contract data through `FSQLUIPersistenceSettingsApplyContractResult` and `FSQLUIPersistenceSettingsCancelPreviewResult`, and accepts `FSQLUIPersistenceSettingsApplyRequest` through `RequestPersistenceSettingsApply` as a callable actual apply entrypoint skeleton. That skeleton evaluates the same contract and returns unavailable/not implemented, validation-blocked, no-change, or preview-only result data while keeping `bSucceeded=false`, `bActualApplyImplemented=false`, and all side-effect flags false. `USQLUIPersistenceSettingsDraftDisplayLibrary`, `USQLUIPersistenceSettingsApplyPreviewDisplayLibrary`, `USQLUIPersistenceSettingsApplyContractDisplayLibrary`, and `USQLUIPersistenceSettingsApplyResultDisplayLibrary` format validation, apply-preview, apply/cancel contract, and apply-result output into UI-safe summary/row data for future sample or settings panels. Apply-result display rows explicitly report safe states such as apply unavailable/not implemented, config not written, settings not changed, provider/repository not initialized, database files not created, directories not created, databases not opened for writing, migrations not run, seeds not copied, and files not deleted. `USQLUISamplePersistenceSettingsDraftPresenter`, `USQLUISamplePersistenceSettingsApplyPreviewPresenter`, `USQLUISamplePersistenceSettingsApplyContractPresenter`, and `USQLUISamplePersistenceSettingsApplyResultPresenter` are SQLUISamples-only sample/dev adapters that consume the validation/apply-preview/apply-contract/apply-result SQLUICore display rows and cache row/summary strings for future sample UI work. `USQLUISamplePersistenceSettingsDraftPanelWidget`, `USQLUISamplePersistenceSettingsApplyPreviewPanelWidget`, `USQLUISamplePersistenceSettingsApplyContractPanelWidget`, and `USQLUISamplePersistenceSettingsApplyResultPanelWidget` are optional SQLUISamples C++ `UUserWidget` shells that delegate to the validation/apply-preview/apply-contract/apply-result presenters and expose cached rows/result/summary/flags for future Blueprint binding. The apply-result shell adds no Apply button, settings controls, lifecycle behavior, widget blueprint asset, viewport attachment, or startup wiring. These helpers do not apply settings, write config, initialize providers/repositories, create directories or database files, open databases for writing, run migrations, copy seed databases, delete files, add settings controls, add widget blueprint assets, attach to the viewport, or change startup behavior. Actual Apply execution is explicitly unavailable/not implemented, and cancel/discard remains value-preview only. The safe binding recipes are documented in [`sqlui_persistence_settings_draft_umg_usage.md`](sqlui_persistence_settings_draft_umg_usage.md) for draft validation, [`sqlui_persistence_settings_apply_preview_umg_usage.md`](sqlui_persistence_settings_apply_preview_umg_usage.md) for dry-run apply-preview display, [`sqlui_persistence_settings_apply_contract_umg_usage.md`](sqlui_persistence_settings_apply_contract_umg_usage.md) for apply/cancel contract display, and [`sqlui_persistence_settings_apply_result_umg_usage.md`](sqlui_persistence_settings_apply_result_umg_usage.md) for apply-result display.

Any future mutating apply/save/config-write behavior belongs behind a SQLUICore policy entrypoint, not in widgets, SQLUISamples presenters, or concrete repository callers. `RequestPersistenceSettingsApply` is only the first callable entrypoint skeleton and deliberately refuses execution without mutation. The first mutating apply path must validate `FSQLUIPersistenceSettingsDraft`, refuse invalid drafts without mutation, write only explicitly scoped persistence settings, avoid committed default config changes in normal runtime/user paths, and return user-readable result/restart/reopen/reinitialize messages. It must not initialize providers/repositories, create DB files or directories, open SQLite for writing, run migrations, copy seeds, delete files, or run reset/delete behavior as a side effect. The detailed implementation gate is maintained in [`sqlui_persistence_settings_editing_reset_plan.md`](sqlui_persistence_settings_editing_reset_plan.md).

`FSQLUIPersistenceSettingsApplyConfigTargetWriter` is the first SQLUICore config-write target scaffold for that future work. It is plain C++ and requires an explicit smoke/test-owned target under `Saved/SQLUI/SmokeTests`; default/runtime targets and unsafe project config paths are refused. It writes only a temporary smoke-owned ini artifact for smoke coverage and does not change the production/default Apply entrypoint, committed config, generated runtime config, active settings, provider/repository lifecycle, SQLite files, migrations, seeds, reset/delete policy, maps, widgets, or startup behavior.

`FSQLUIPersistenceSettingsApplyConfigTargetPolicy` is the #134 non-mutating resolver layer above that scaffold. It makes target selection explicit without enabling production Apply: default/runtime resolves unavailable and non-writable, explicit safe smoke-owned targets resolve write-capable for smoke/test coverage only, and the future project/user config target resolves unavailable/not implemented until a later PR chooses and validates a real target. `ResolveDocumentedProductionTargetStrategy()` now codifies the documented production target strategy as the same future project/user target kind with no writable path, `bCanWrite=false`, and production Apply disabled. `RequestPersistenceSettingsApply` remains the existing unavailable/non-mutating apply entrypoint and still keeps every side-effect flag false.

The #132-#134 checkpoints plus the documented-strategy resolver prove target isolation only. They do not make `RequestPersistenceSettingsApply` a production Apply path and do not add settings controls or lifecycle ownership to SQLUISamples. The smoke-owned writer proves that SQLUICore can validate a draft before serialization, refuse invalid drafts without mutating the smoke target, distinguish explicit smoke/test targets from default/runtime targets, keep repo `Config` and `Saved/Config` unchanged, and clean up the temporary artifact. The policy/resolver checkpoint proves that the only writable target is still the explicit smoke-owned target; production/default, documented production strategy, and future project/user targets remain unavailable. Any future production/user config-write path must choose a safe real target explicitly, keep writes narrow, validate drafts, preserve no-op behavior, keep `InMemory` as the default, keep SQLite and provider auto-init opt-in, avoid provider/repository initialization and DB work, and include config diff/snapshot coverage before UI controls are added.

The production config target strategy gate now records and SQLUICore policy resolves that no existing real target is selected. `USQLUILayoutRepositoryRuntimeSettings` is the existing config-backed settings policy surface, but writing it from runtime Apply is not enabled because it needs explicit target semantics and startup impact validation. Committed defaults, user/global editor settings, widget-owned files, and implicit `Saved/Config` writes are rejected or not selected. A future real target should be SQLUICore-owned, deliberately scoped, and validated before any Apply path can mutate it; until then production Apply remains unavailable and smoke-owned targets remain the only write-capable path.
...
The read-only persistence status panel contract is documented in [`sqlui_persistence_settings_ux_design.md`](sqlui_persistence_settings_ux_design.md), with the focused UMG binding recipe in [`sqlui_persistence_status_umg_usage.md`](sqlui_persistence_status_umg_usage.md). The draft validation widget-shell recipe is documented in [`sqlui_persistence_settings_draft_umg_usage.md`](sqlui_persistence_settings_draft_umg_usage.md), the dry-run apply-preview widget-shell recipe is documented in [`sqlui_persistence_settings_apply_preview_umg_usage.md`](sqlui_persistence_settings_apply_preview_umg_usage.md), the apply/cancel contract widget-shell recipe is documented in [`sqlui_persistence_settings_apply_contract_umg_usage.md`](sqlui_persistence_settings_apply_contract_umg_usage.md), and the apply-result widget-shell recipe is documented in [`sqlui_persistence_settings_apply_result_umg_usage.md`](sqlui_persistence_settings_apply_result_umg_usage.md). Together they define how future Blueprint/UMG UI should consume the existing widget-shell/adapter/presenter/display-row path: show row labels/values, optionally map row state/detail text to UI affordances, keep refresh/build caller-invoked only, use SQLUICore preview/contract/result data and display rows only as dry-run/refusal report data, and keep widgets away from provider lifecycle, direct file checks, migrations, seed-copy policy, apply/save behavior, and reset/delete behavior. They are usage recipes plus optional sample shells, not finished settings screens.

That read-only persistence status UMG foundation is complete as a sample/dev-facing binding base. The non-mutating persistence settings draft validation UI foundation is also complete through the #105 plan, #106 draft model, #107 validation display rows, #108 sample adapter, #109 C++ UMG shell, #110 usage guide, and #111 final checkpoint. The non-mutating apply-preview UI foundation is complete through #112 dry-run apply-intent preview, #113 UI-safe apply-preview display rows/summary, #114 optional SQLUISamples apply-preview sample adapter, #115 optional SQLUISamples apply-preview C++ UMG widget shell, #116 focused apply-preview UMG usage guide, and #117 final checkpoint. The non-mutating apply/cancel contract UI foundation is complete through #118 apply/cancel contract readiness and cancel/discard preview, #119 UI-safe apply/cancel contract display rows/summary, #120 editor validation checkpoint, #121 optional SQLUISamples apply/cancel contract sample adapter, #122 optional apply/cancel contract C++ UMG widget shell, #123 focused apply/cancel contract UMG usage guide, #124 final docs-only foundation checkpoint, and `-UsePersistenceSettingsDraftProbe` smoke coverage. The non-mutating apply entrypoint/result UI foundation is complete through #125 actual apply implementation gate, #126 SQLUICore apply entrypoint/result skeleton, #127 UI-safe apply-result display rows/summary, #128 optional SQLUISamples apply-result sample adapter, #129 optional apply-result C++ UMG widget shell, #130 apply-result UMG usage guide, #131 final docs-only apply-result foundation checkpoint, and the same draft probe coverage. The first actual apply entrypoint skeleton is present in SQLUICore, but it deliberately returns unavailable/not implemented and performs no mutation; its apply result now has UI-safe display rows/summary plus SQLUISamples sample/dev presenter and C++ UMG shell surfaces for future settings UI. These foundations do not change repository selection, default startup, default maps/config, provider auto-init policy, SQLite defaults, or packaged runtime behavior. Future settings-editing, backend-selection, SQLite-path editing, actual mutating apply/cancel execution, provider auto-init controls, migration/seed-copy controls, and reset/delete UI must remain above SQLUICore helper/policy surfaces, follow [`sqlui_persistence_settings_editing_reset_plan.md`](sqlui_persistence_settings_editing_reset_plan.md), and use pending/apply semantics rather than widget-owned persistence mutation. Widgets must not write config directly, initialize providers/repositories directly, create databases or directories directly, run migrations, copy seeds, or delete files directly.
...
- Persistence settings draft probe: SQLUISamples exercises `USQLUIPersistenceSettingsDraftLibrary`, `USQLUIPersistenceSettingsDraftDisplayLibrary`, `USQLUIPersistenceSettingsApplyPreviewDisplayLibrary`, `USQLUIPersistenceSettingsApplyContractDisplayLibrary`, `USQLUIPersistenceSettingsApplyResultDisplayLibrary`, `USQLUISamplePersistenceSettingsDraftPresenter`, `USQLUISamplePersistenceSettingsApplyPreviewPresenter`, `USQLUISamplePersistenceSettingsApplyContractPresenter`, `USQLUISamplePersistenceSettingsApplyResultPresenter`, `USQLUISamplePersistenceSettingsDraftPanelWidget`, `USQLUISamplePersistenceSettingsApplyPreviewPanelWidget`, `USQLUISamplePersistenceSettingsApplyContractPanelWidget`, and `USQLUISamplePersistenceSettingsApplyResultPanelWidget`, verifies default/current draft validation and display rows, dry-run apply-preview no-change behavior, non-mutating apply/cancel contract reporting and display rows, unavailable/non-mutating actual apply request skeleton behavior and apply-result display rows/adapter/widget-shell contract, validation/apply-preview/apply-contract/apply-result sample adapter consumption, validation/apply-preview/apply-contract/apply-result C++ UMG widget shell reflection contracts, unknown backend rejection/error display/blocked preview/blocked apply contract/contract display/apply request/apply-result display/adapter/widget contract, SQLite draft/path display and preview without DB creation, empty SQLite path rejection/error display/blocked preview/blocked apply contract/contract display, provider auto-init pending validation/display/preview/contract/contract display/apply request/apply-result display/adapter/widget contract without policy mutation, deterministic validation/display/preview/preview-display/contract-display/apply-result-display/apply-result-adapter/apply-result-widget-contract/contract/apply-request/adapter output, sidecar preservation, config preservation, and cleanup under `Saved/SQLUI/SmokeTests/PersistenceSettingsDraft`.
...
4. Implement the next persistence settings slice from [`sqlui_persistence_settings_ux_design.md`](sqlui_persistence_settings_ux_design.md) and [`sqlui_persistence_settings_editing_reset_plan.md`](sqlui_persistence_settings_editing_reset_plan.md), building on the non-mutating draft validation model, apply-intent preview, apply/cancel contract, actual apply entrypoint skeleton, apply-result display rows, apply-result presenter, apply-result widget shell, display rows, sample presenters, and widget shells before adding mutating Apply execution or editing controls.
