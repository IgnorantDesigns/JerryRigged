...
Any future actual apply/save/config-write behavior belongs behind a SQLUICore policy entrypoint, not in widgets, SQLUISamples presenters, or concrete repository callers. The first mutating apply path must validate `FSQLUIPersistenceSettingsDraft`, refuse invalid drafts without mutation, write only explicitly scoped persistence settings, avoid committed default config changes in normal runtime/user paths, and return user-readable result/restart/reopen/reinitialize messages. It must not initialize providers/repositories, create DB files or directories, open SQLite for writing, run migrations, copy seeds, delete files, or run reset/delete behavior as a side effect. The detailed implementation gate is maintained in [`sqlui_persistence_settings_editing_reset_plan.md`](sqlui_persistence_settings_editing_reset_plan.md).
