...
`USQLUIPersistenceSettingsDraftLibrary` is the first non-mutating SQLUICore settings draft helper for future settings panels. It builds current/default draft data from runtime settings policy, represents pending runtime config and provider auto-init values, validates backend/path/provider-auto-init choices, and can reset a draft value back to current values. It does not apply settings, write config, initialize providers/repositories, create directories or database files, run migrations, copy seed databases, delete files, add UI controls, or change startup behavior.
...
- Persistence settings draft probe: SQLUISamples exercises `USQLUIPersistenceSettingsDraftLibrary`, verifies default/current draft validation, unknown backend rejection, SQLite draft representation without DB creation, empty SQLite path rejection, provider auto-init pending validation without policy mutation, deterministic validation, sidecar preservation, and cleanup under `Saved/SQLUI/SmokeTests/PersistenceSettingsDraft`.
