...
- Persistence status sample surface probe: SQLUISamples exercises `USQLUISamplePersistenceStatusPresenter`, verifies the presenter refresh functions and result are reflected for Blueprint use, verifies explicit and repeated refresh of default `InMemory` rows and formatted lines without initializing a provider or repository, verifies repeated refresh is deterministic and does not create a DB, verifies an explicit missing SQLite path is shown gracefully without creating a DB, verifies a smoke-owned sidecar is not deleted by refresh, and cleans up under `Saved/SQLUI/SmokeTests/PersistenceStatusSampleSurface`.
