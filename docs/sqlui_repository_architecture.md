...
`FSQLUIPersistenceSettingsApplyConfigTargetPolicy` is the non-mutating resolver layer above that scaffold. It makes target selection explicit without enabling production Apply: default/runtime resolves unavailable and non-writable, explicit safe smoke-owned targets resolve write-capable for smoke/test coverage only, and the future project/user config target resolves unavailable/not implemented until a later PR chooses and validates a real target. `RequestPersistenceSettingsApply` consults the default target policy and still keeps every side-effect flag false.
