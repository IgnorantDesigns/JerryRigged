...
The production config target resolution and guarded enablement checkpoints are in that same non-packaged category. They document that SQLUICore can represent the future target and an explicit enablement request while keeping the result unavailable/non-writable/pathless, and they do not change packaged startup, default maps, runtime config loading, provider lifecycle, or packaged DB path behavior. Future real config-write work or startup/default-map/provider lifecycle behavior changes should run packaged validation and preserve the MSVC toolchain guidance below.
