...
- Packaged validation for any future real runtime/user config-write Apply behavior that changes startup/config/provider lifecycle semantics. The current apply config target scaffold, target policy/resolver skeleton, and production config target strategy resolution are smoke-owned/non-mutating or non-writable policy-only work and do not require packaged validation by themselves.
