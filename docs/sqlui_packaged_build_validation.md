...
- Packaged validation for any future real runtime/user config-write Apply behavior that changes startup/config/provider lifecycle semantics. The current apply config target scaffold, target policy/resolver skeleton, production config target strategy resolution/descriptor, guarded enablement request, backend-only selected-target write, and explicit backend-only selected-target readback are smoke-owned, non-mutating, policy-only, or explicitly backend-only commandlet-smoke work that do not require packaged validation by themselves because they do not change startup, default maps, provider lifecycle, runtime config loading, or packaged DB path behavior.
