...
- Packaged validation for any future real runtime/user config-write Apply behavior that changes startup/config/provider lifecycle semantics. The current apply config target scaffold and target policy/resolver skeleton are smoke-owned/non-mutating only and do not require packaged validation by themselves.
