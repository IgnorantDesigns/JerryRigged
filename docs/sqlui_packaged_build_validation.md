...
The read-only persistence status UMG foundation, docs-only settings editing/reset planning, validation-only persistence settings draft model, dry-run apply-intent preview, non-mutating apply/cancel contract, unavailable/non-mutating actual apply entrypoint skeleton, UI-safe draft validation display rows, UI-safe apply-preview display rows, UI-safe apply/cancel contract display rows, UI-safe apply-result display rows, SQLUISamples draft validation/apply-preview/apply-contract/apply-result sample adapters, optional validation/apply-preview/apply-contract/apply-result C++ UMG widget shells, docs-only draft/apply-preview/apply-contract/apply-result UMG usage guides including #116, #123, and #130, #111 docs-only draft validation foundation checkpoint, #117 docs-only apply-preview UI foundation checkpoint, the #124 docs-only apply/cancel contract UI foundation checkpoint, the #125 docs-only actual apply implementation gate, the docs-only #105-#131 apply entrypoint/result UI foundation checkpoint, the #132 explicit smoke-owned apply config target scaffold, the #134 non-mutating apply config target policy/resolver skeleton, docs-only checkpoint updates for that scaffold, and the production config target strategy/resolution/design/descriptor gate do not add packaged startup behavior. Docs-only checkpoint/planning/usage/strategy/design PRs and non-mutating validation/preview/contract/result-display-only PRs do not require a full packaged build when they do not touch maps, config, startup wiring, provider lifecycle, viewport attachment, or packaged lifecycle behavior. The #132 scaffold writes only an explicit temporary `Saved/SQLUI/SmokeTests` ini target during editor commandlet smoke coverage and cleans it up; the #134 resolver skeleton only reports default/runtime unavailable, smoke-owned explicit-only, and future real target unavailable; the documented production strategy resolver reports a non-writable future target, carries the selected `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` descriptor without creating it, and keeps production Apply unavailable; the guarded enablement resolver can record an explicit production target enablement request but still reports it blocked/non-writable. None of those changes packaged startup/config/provider behavior by themselves. Future PRs that implement the selected target, add real user/runtime config writes, change startup behavior, or wire persistence status UI or mutating settings/reset UI into startup, default maps, config, viewport attachment, provider lifecycle, or packaged runtime flows should include packaged validation appropriate to that behavior.

The production config target resolution, PR #139 guarded enablement request, and the descriptor checkpoint are in that same non-packaged category. They document that SQLUICore can represent the future target, that the selected storage design is `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini`, and that explicit enablement requests still return unavailable/non-writable results until implementation. They do not create the target file/directory and do not change packaged startup, default maps, runtime config loading, provider lifecycle, or packaged DB path behavior. Future real config-write work or startup/default-map/provider lifecycle behavior changes should run packaged validation and preserve the MSVC toolchain guidance below.

The first actual apply/save/config-write implementation should treat packaged validation as required if it can affect startup, config-backed provider lifecycle, default maps, viewport wiring, packaged runtime settings, or persisted runtime behavior. If that implementation remains a SQLUICore-only not-implemented/dry-run/apply-refusal entrypoint with no config writes or lifecycle changes, editor smoke coverage is usually sufficient.
...
- Packaged validation for any future real runtime/user config-write Apply behavior that changes startup/config/provider lifecycle semantics. The current apply config target scaffold, target policy/resolver skeleton, production config target strategy resolution/descriptor, and guarded enablement request are smoke-owned/non-mutating or non-writable policy-only work and do not require packaged validation by themselves.
