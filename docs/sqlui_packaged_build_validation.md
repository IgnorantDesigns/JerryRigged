...
- Packaged validation for any future real runtime/user config-write Apply behavior that changes startup/config/provider lifecycle semantics. The current apply config target scaffold, target policy/resolver skeleton, and production config target strategy gate are smoke-owned/non-mutating/docs-only and do not require packaged validation by themselves.
