...
The actual apply implementation gate in `docs/sqlui_persistence_settings_editing_reset_plan.md` requires any first mutating apply/config-write smoke path to stay SQLUICore-first and smoke-owned. The current production/default apply entrypoint skeleton is covered by `-UsePersistenceSettingsDraftProbe` and deliberately returns unavailable/not implemented without mutation. The same probe now also covers an explicit smoke-owned config target scaffold, a non-mutating target policy/resolver skeleton, and the documented production target strategy resolution: default/runtime remains unavailable and non-writable, explicit safe smoke-owned targets can write only to a temporary `Saved/SQLUI/SmokeTests` ini file and clean it up, the future project/user target remains unavailable/not implemented, and the documented production strategy exposes no writable path. This docs-only target design now selects `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` as the future SQLUICore-owned target, but no code writes it yet. Real user/runtime Apply remains unavailable: no committed config or `Saved/Config` file is written, no runtime settings are changed, no DB files are created, no migrations or seed copy run, and no provider/repository lifecycle work runs. Future mutating smoke coverage should prove real valid backend-only apply in the selected target, target path resolution, invalid draft refusal with no mutation, no-change/no-op behavior, smoke-owned versus production target separation, default config safety, and no side-effect DB creation, migrations, seed copy, provider initialization, repository initialization, reset, or file deletion. No separate actual apply/config-write smoke flag exists for this design checkpoint.

The production config target strategy resolution adds no new smoke flag. Existing probes already confirm smoke-owned target behavior, target policy behavior, the documented strategy's non-writable/no-writable-path status, and the selected production descriptor for `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini`. The guarded production target enablement request is covered by the same draft probe: an explicit request is recorded, but remains blocked, non-writable, deterministic, and side-effect free because the selected target is descriptor-only and not implemented for writes. A future PR that enables that target must add explicit new or extended smoke assertions for target write behavior, smoke-owned versus real-target separation, committed `Config` preservation, generated `Saved/Config` diff/restore behavior if relevant, valid backend-only writes, invalid-draft refusal, no-change/no-op behavior, no DB creation, no DB write-open, no migrations, no seed copy, no provider/repository lifecycle work, and cleanup of any smoke-owned or real-target test artifacts.

The PR #138 production config target resolution checkpoint and PR #139 guarded production target enablement checkpoint use the existing `-UsePersistenceSettingsDraftProbe` path and add no new smoke flag. The probe covers the validation-only draft model, display rows, dry-run apply preview, apply/cancel contract, apply entrypoint skeleton refusal/no-mutation behavior, apply-result display/adapter/widget paths, smoke-owned apply config target scaffold, apply config target policy/resolver skeleton, documented production config target resolution, the selected production target descriptor, and guarded production target enablement request behavior. It verifies default/runtime Apply remains unavailable/not implemented, the smoke-owned target remains explicit and isolated, the documented future real project/user target remains unavailable/non-writable with no writable path, explicit production target enablement requests are recorded but blocked/non-writable, the selected production target file/directory are not created, repo `Config` / `Saved/Config` snapshots remain unchanged, cleanup removes only smoke-owned files, no draft/status/apply probe DB or sidecar files remain, and no smoke-owned apply config target artifact remains. This descriptor checkpoint changes target description only; it adds no new smoke command and keeps production writes blocked until an implementation PR writes the selected `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` target.
...
It also covers the first smoke-owned apply config target scaffold from #132 and the #134 SQLUICore target policy/resolver skeleton: the production/default apply target is refused, default/runtime policy cannot write, unsafe project config paths are refused, a valid draft can write only to a temporary `Saved\SQLUI\SmokeTests` ini target, invalid drafts do not mutate that target, future project/user target policy remains unavailable/not implemented, the documented production target strategy resolves as non-writable with no implemented production write path, the selected production descriptor points to `Saved\SQLUI\PersistenceSettings\RuntimeSettings.ini` without creating it, the guarded production target enablement request is recorded but blocked/non-writable, repo `Config` and `Saved\Config` snapshots remain unchanged, no database/lifecycle/migration/seed-copy work runs, and the smoke-owned target artifact is removed before the probe reports success.
...
This path proves validation/preview-only draft behavior, non-mutating apply/cancel contract reporting, the unavailable/non-mutating actual apply request skeleton, the smoke-owned apply config target scaffold, the selected production target descriptor, UI-safe validation display-row formatting and summary generation, UI-safe apply-preview display-row formatting and summary generation, UI-safe apply/cancel contract display-row formatting and summary generation, UI-safe apply-result display-row formatting and summary generation, SQLUISamples sample adapter consumption of validation, apply-preview, apply/cancel contract, and apply-result rows, and the optional validation/apply-preview/apply-contract/apply-result C++ UMG widget shell binding contracts. The future Blueprint/UMG binding recipes live in [`sqlui_persistence_settings_draft_umg_usage.md`](sqlui_persistence_settings_draft_umg_usage.md) for validation, [`sqlui_persistence_settings_apply_preview_umg_usage.md`](sqlui_persistence_settings_apply_preview_umg_usage.md) for apply preview, [`sqlui_persistence_settings_apply_contract_umg_usage.md`](sqlui_persistence_settings_apply_contract_umg_usage.md) for apply/cancel contract, and [`sqlui_persistence_settings_apply_result_umg_usage.md`](sqlui_persistence_settings_apply_result_umg_usage.md) for apply result. Each shell follows the same caller-invoked, display-only pattern and still has no real Apply behavior. The smoke-owned config target scaffold is not the production Apply path, and the selected `Saved\SQLUI\PersistenceSettings\RuntimeSettings.ini` production target is described only, not created. It does not require widget blueprint assets or maps, does not attach anything to the viewport, does not alter startup/config behavior, does not apply settings to runtime state, does not write project/user/default config, does not add settings editing UI controls, add backend selector controls, add SQLite path editor controls, add provider auto-init toggle controls, add reset/delete behavior, add widget blueprint assets, add visual layout, initialize providers or repositories, create database files, create DB directories, open databases for writing, run migrations, copy seed databases, delete files outside smoke-owned cleanup, change startup behavior, edit Content or maps, or leave persistent database files behind. The only directory/file creation in this smoke path is the explicit smoke-owned sidecar fixture used to verify cleanup and non-deletion behavior plus the temporary smoke-owned apply config target artifact under `Saved\SQLUI\SmokeTests`.

After the probe succeeds, database files under `Saved\SQLUI\SmokeTests\PersistenceSettingsDraft` should not exist, and the temporary apply config target directory under `Saved\SQLUI\SmokeTests\PersistenceSettingsDraft\ApplyConfigTarget` should be gone. The selected production descriptor target `Saved\SQLUI\PersistenceSettings\RuntimeSettings.ini` and its parent directory should not be created by this probe. A full checkpoint cleanup check should also confirm that the related read-only status paths used by this phase, such as `Saved\SQLUI\SmokeTests\PersistenceStatusSampleSurface` and `Saved\SQLUI\SmokeTests\PersistenceStatusDisplayRows`, contain no smoke-owned `.db`, `.db-journal`, `.db-wal`, or `.db-shm` files.
