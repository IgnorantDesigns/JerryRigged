...
The mutating settings editing and reset phase is planned in `docs/sqlui_persistence_settings_editing_reset_plan.md`. The first implementation slices add `-UsePersistenceSettingsDraftProbe` for non-mutating draft/pending settings validation, UI-safe validation display rows, an optional SQLUISamples sample/dev adapter that consumes those rows, and an optional C++ UMG widget shell contract for future Blueprint binding. Future implementation PRs that add apply behavior, backend selector UI, SQLite path editor UI, provider auto-init controls, or reset/delete actions should add focused smoke coverage for those behaviors while preserving the existing read-only status and draft-validation semantics.
