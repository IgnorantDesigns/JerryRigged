...
The actual apply implementation gate in `docs/sqlui_persistence_settings_editing_reset_plan.md` requires any first mutating apply/config-write smoke path to stay SQLUICore-first and smoke-owned. The current production/default apply entrypoint skeleton is covered by `-UsePersistenceSettingsDraftProbe` and deliberately returns unavailable/not implemented without mutation. The same probe now also covers an explicit smoke-owned config target scaffold, a non-mutating target policy/resolver skeleton, and the documented production target strategy resolution: default/runtime remains unavailable and non-writable, explicit safe smoke-owned targets can write only to a temporary `Saved/SQLUI/SmokeTests` ini file and clean it up, the future project/user target remains unavailable/not implemented, and the documented production strategy exposes no writable path. Real user/runtime Apply remains unavailable: no committed config or `Saved/Config` file is written, no runtime settings are changed, no DB files are created, no migrations or seed copy run, and no provider/repository lifecycle work runs. Future mutating smoke coverage should prove real valid apply in an isolated config context, invalid draft refusal with no mutation, no-change/no-op behavior, smoke-owned config cleanup/restore, default config safety, and no side-effect DB creation, migrations, seed copy, provider initialization, repository initialization, reset, or file deletion. No separate actual apply/config-write smoke flag exists for this scaffold.

The production config target strategy resolution adds no new smoke flag. Existing probes already confirm smoke-owned target behavior, target policy behavior, and the documented strategy's non-writable/no-path status. The guarded production target enablement request is covered by the same draft probe: an explicit request is recorded, but remains blocked, non-writable, pathless, deterministic, and side-effect free because no safe concrete production target is selected yet. A future PR that enables any real target must add explicit new or extended smoke assertions for smoke-owned versus real-target separation, committed `Config` preservation, generated `Saved/Config` diff/restore behavior if relevant, no DB creation, no DB write-open, no migrations, no seed copy, no provider/repository lifecycle work, invalid-draft refusal, no-change/no-op behavior, and cleanup of any smoke-owned or real-target test artifacts.
