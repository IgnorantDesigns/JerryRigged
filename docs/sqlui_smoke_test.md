...
The mutating settings editing and reset phase is planned in `docs/sqlui_persistence_settings_editing_reset_plan.md`. The non-mutating draft validation UI foundation is complete through `-UsePersistenceSettingsDraftProbe`: validation-only draft model, UI-safe validation display rows/summary, optional SQLUISamples sample/dev adapter, and optional C++ draft validation UMG widget shell contract for future Blueprint binding. The safe binding recipe for that draft widget shell is documented in `docs/sqlui_persistence_settings_draft_umg_usage.md`. Future implementation PRs that add apply behavior, backend selector UI, SQLite path editor UI, provider auto-init controls, or reset/delete actions should add focused smoke coverage for those behaviors while preserving the existing read-only status and draft-validation semantics.
