...
The mutating settings editing and reset phase is planned in `docs/sqlui_persistence_settings_editing_reset_plan.md`. The non-mutating draft validation UI foundation is complete through the #105-#111 sequence and `-UsePersistenceSettingsDraftProbe`: validation-only draft model, UI-safe validation display rows/summary, SQLUISamples draft validation adapter, and C++ draft validation UMG widget shell contract for future Blueprint binding. The same draft probe also covers the SQLUICore dry-run apply-intent preview, UI-safe apply-preview display rows/summary, SQLUISamples apply-preview adapter, C++ apply-preview UMG widget shell contract, SQLUICore non-mutating apply/cancel contract or apply-readiness preview, UI-safe apply/cancel contract display rows/summary, SQLUISamples apply/cancel contract adapter, C++ apply/cancel contract UMG widget shell contract, the unavailable/non-mutating actual apply request skeleton, UI-safe apply-result display rows/summary, SQLUISamples apply-result sample adapter, C++ apply-result UMG widget shell contract, the #124 apply/cancel contract checkpoint, the completed #105-#131 non-mutating apply entrypoint/result UI foundation checkpoint, and the first explicit smoke-owned apply config target scaffold. It does this without widget Blueprint assets, maps, viewport attachment, startup/config behavior changes, provider/repository initialization, runtime settings mutation, committed config writes, DB creation, migrations, seed copy, or file deletion outside smoke-owned cleanup. The safe binding recipes for those shells are documented in `docs/sqlui_persistence_settings_draft_umg_usage.md` for draft validation, `docs/sqlui_persistence_settings_apply_preview_umg_usage.md` for dry-run apply-preview display, `docs/sqlui_persistence_settings_apply_contract_umg_usage.md` for apply/cancel contract display, and `docs/sqlui_persistence_settings_apply_result_umg_usage.md` for apply-result display. Future implementation PRs that add mutating actual apply behavior, backend selector UI, SQLite path editor UI, provider auto-init controls, or reset/delete actions should add focused smoke coverage for those behaviors while preserving the existing read-only status, draft-validation, apply-preview, apply/cancel contract, apply-request-refusal, apply-result display, apply-result adapter, apply-result widget shell, and smoke-owned config target semantics.

The actual apply implementation gate in `docs/sqlui_persistence_settings_editing_reset_plan.md` requires mutating apply/config-write smoke paths to stay SQLUICore-first and narrowly scoped. The current production/default apply path is covered by `-UsePersistenceSettingsDraftProbe` and deliberately returns unavailable/not implemented without mutation unless the request explicitly asks for the backend-only production write. The same probe also covers the explicit smoke-owned config target scaffold, target policy/resolver skeleton, documented production target strategy resolution, the first backend-only write to `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini`, and PR #145's explicit backend-only readback from that file. Default/runtime remains unavailable and non-writable, explicit safe smoke-owned targets can write only to a temporary `Saved/SQLUI/SmokeTests` ini file and clean it up, and the selected production target can be touched only by an explicit backend-only request that serializes no SQLite path, provider auto-init, migration, seed, reset, or delete settings. Backend-only readback is caller-invoked only, reads just `Backend`, treats an absent file as no override, reports unknown/missing backend content as errors, and does not apply runtime settings. No committed config or `Saved/Config` file is written, no live runtime settings/provider/repository state changes, no DB files are created, no migrations or seed copy run, and no provider/repository lifecycle work runs. No separate actual apply/config-write smoke flag exists for this backend-only checkpoint.

The production config target strategy resolution adds no new smoke flag. Existing probes confirm smoke-owned target behavior, target policy behavior, the documented strategy's non-writable/no-writable-path status, the selected production descriptor for `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini`, backend-only write behavior, and backend-only readback behavior. The guarded production target enablement request is covered by the same draft probe: an explicit resolver request is recorded but remains policy-only, while the actual write path is the explicit backend-only apply request and the read path is an explicit backend-only parser. Future PRs that add more settings must add explicit or extended smoke assertions for target write/read behavior, smoke-owned versus real-target separation, committed `Config` preservation, generated `Saved/Config` diff/restore behavior if relevant, invalid-draft refusal, no-change/no-op behavior, no DB creation, no DB write-open, no migrations, no seed copy, no provider/repository lifecycle work, and cleanup of any smoke-owned or real-target test artifacts.

The PR #138 production config target resolution checkpoint, PR #139 guarded production target enablement checkpoint, backend-only production write checkpoint, and PR #145 backend-only runtime settings readback checkpoint use the existing `-UsePersistenceSettingsDraftProbe` path and add no new smoke flag. The probe covers the validation-only draft model, display rows, dry-run apply preview, apply/cancel contract, default apply refusal/no-mutation behavior, apply-result display/adapter/widget paths, smoke-owned apply config target scaffold, apply config target policy/resolver skeleton, documented production config target resolution, the selected production target descriptor, guarded production target enablement request behavior, backend-only production apply behavior, and backend-only readback behavior. It verifies default/runtime Apply remains unavailable/not implemented, the smoke-owned target remains explicit and isolated, the broad production resolver remains unavailable/non-writable with no general writable path, explicit production target enablement resolver requests are recorded but policy-only, backend-only writes serialize just `Backend`, readback treats absence as no override, valid readback parses only `Backend`, invalid/missing backend content is rejected without mutation, invalid/no-change/provider-auto-init requests do not mutate the target, repo `Config` / `Saved/Config` snapshots remain unchanged, cleanup removes smoke-owned and probe-created production files, no draft/status/apply probe DB or sidecar files remain, and no smoke-owned apply config target artifact remains.
...
The persistence settings draft probe keeps the same transient commandlet flow, exercises `USQLUIPersistenceSettingsDraftLibrary`, `USQLUIPersistenceSettingsDraftDisplayLibrary`, `USQLUIPersistenceSettingsApplyPreviewDisplayLibrary`, `USQLUIPersistenceSettingsApplyContractDisplayLibrary`, `USQLUIPersistenceSettingsApplyResultDisplayLibrary`, the SQLUISamples draft validation sample adapter, the SQLUISamples apply-preview sample adapter, the SQLUISamples apply/cancel contract sample adapter, the SQLUISamples apply-result sample adapter, and the optional validation/apply-preview/apply-contract/apply-result C++ UMG widget shell contracts. It verifies the default/current `InMemory` draft validates safely, produces safe validation display rows, and reports no changes in the dry-run apply-intent preview, non-mutating apply/cancel contract, actual apply request skeleton, apply-result display rows/adapter/widget-shell contract, apply-preview display rows/adapters, and apply/cancel contract display rows/adapter; verifies an unknown backend is rejected with validation messages, a user-readable validation error row, a blocked apply preview, a blocked apply contract, blocked apply request, blocked apply-result display/adapter/widget display contract, user-readable apply-preview error rows, and user-readable apply/cancel contract display rows/adapter rows; verifies a pending SQLite draft/path can be represented and previewed without applying it or creating a database; verifies the apply request skeleton, apply-result display, apply-result adapter, and apply-result widget shell contract report SQLite as preview-only and do not create DB files or directories; verifies an empty SQLite path is rejected safely and previewed/displayed as blocked; verifies a pending provider auto-init value validates, displays, and previews/displays as pending without changing provider auto-init policy; verifies the apply request skeleton, apply-result display, apply-result adapter, and apply-result widget shell contract do not change provider auto-init policy or config files; verifies cancel preview reports pending changes as discardable value state only; verifies the widget shells derive from `UUserWidget`; verifies their caller-invoked refresh/build hooks are Blueprint-callable but not pure; verifies cached getters/properties are Blueprint-visible as intended; verifies the shell contracts by reflection without a widget blueprint asset, map, viewport instance, or startup wiring; verifies repeated validation/display/preview/apply-preview display/apply-contract display/apply-result display/apply-result adapter/apply-result widget contract/apply-contract/apply-request/cancel-preview and adapter output is deterministic; verifies validation, preview, preview-display, apply-preview adapter, apply-contract display, apply-result display, apply-result adapter, apply-result widget shell contract, apply-contract adapter, apply-contract, apply-request, and cancel-preview generation do not delete a smoke-owned SQLite sidecar; removes all probe files; and then runs the same default runtime widget pipeline:
...
It also covers the first smoke-owned apply config target scaffold from #132 and the #134 SQLUICore target policy/resolver skeleton: the production/default apply target is refused, default/runtime policy cannot write, unsafe project config paths are refused, a valid draft can write only to a temporary `Saved\SQLUI\SmokeTests` ini target, invalid drafts do not mutate that target, broad project/user target policy remains unavailable/not implemented, the documented production target strategy resolves as non-writable with no general production write path, the selected production descriptor points to `Saved\SQLUI\PersistenceSettings\RuntimeSettings.ini` without creating it by descriptor resolution alone, the guarded production target enablement request is recorded at the resolver layer, backend-only production apply can create/write that selected file with only `Backend`, PR #145 backend-only readback can parse only that backend value when called explicitly, repo `Config` and `Saved\Config` snapshots remain unchanged, no database/lifecycle/migration/seed-copy work runs, and smoke-owned plus probe-created production target artifacts are removed before the probe reports success.
...
- SQLUISamples apply-result adapter.
...
- SQLUICore documented production config target strategy resolution.
- SQLUICore guarded production config target enablement request resolution.
- SQLUICore backend-only selected runtime settings readback.
...
- Backend-only runtime settings readback treats absent `RuntimeSettings.ini` as no override, reads only `Backend` after the smoke-owned backend-only write, rejects unknown or missing backend content, applies nothing to active runtime/provider/repository state, reads no SQLite path/provider auto-init values, creates no files or directories, and cleans up probe-created target artifacts.
...
SQLUI persistence settings draft probe apply result adapter default display generated: true
SQLUI persistence settings draft probe apply result adapter default display safe: true
SQLUI persistence settings draft probe apply result adapter current display no changes: true
SQLUI persistence settings draft probe apply result adapter execution unavailable: true
SQLUI persistence settings draft probe apply result adapter unknown backend shows error: true
SQLUI persistence settings draft probe apply result adapter SQLite display generated: true
SQLUI persistence settings draft probe apply result adapter SQLite display did not create DB: true
SQLUI persistence settings draft probe apply result adapter provider auto-init pending: true
SQLUI persistence settings draft probe apply result adapter repeated display deterministic: true
SQLUI persistence settings draft probe apply result adapter preserved config files: true
SQLUI persistence settings draft probe apply result adapter did not create directory: true
SQLUI persistence settings draft probe apply result panel widget class derived from UUserWidget: true
SQLUI persistence settings draft probe apply result panel widget Blueprint default refresh function callable: true
SQLUI persistence settings draft probe apply result panel widget Blueprint current refresh function callable: true
SQLUI persistence settings draft probe apply result panel widget Blueprint build function callable: true
SQLUI persistence settings draft probe apply result panel widget refresh functions not BlueprintPure: true
SQLUI persistence settings draft probe apply result panel widget cached getter functions BlueprintPure: true
SQLUI persistence settings draft probe apply result panel widget rows property Blueprint visible: true
SQLUI persistence settings draft probe apply result panel widget formatted lines property Blueprint visible: true
SQLUI persistence settings draft probe apply result panel widget refresh result property Blueprint visible: true
SQLUI persistence settings draft probe apply result panel widget summary text property Blueprint visible: true
SQLUI persistence settings draft probe apply result panel widget apply result flags Blueprint visible: true
SQLUI persistence settings draft probe apply result panel widget contract validated without asset or viewport: true
SQLUI persistence settings draft probe apply result panel widget default display generated: true
SQLUI persistence settings draft probe apply result panel widget default display safe: true
SQLUI persistence settings draft probe apply result panel widget unknown backend shows error: true
SQLUI persistence settings draft probe apply result panel widget SQLite display did not create DB: true
SQLUI persistence settings draft probe apply result panel widget provider auto-init pending: true
SQLUI persistence settings draft probe apply result panel widget repeated display deterministic: true
SQLUI persistence settings draft probe apply result panel widget preserved config files: true
SQLUI persistence settings draft probe apply result panel widget did not create directory: true
SQLUI persistence settings draft probe apply config target default runtime unavailable: true
SQLUI persistence settings draft probe apply config target smoke target validated: true
SQLUI persistence settings draft probe apply config target valid draft wrote smoke config: true
SQLUI persistence settings draft probe apply config target recorded expected values: true
SQLUI persistence settings draft probe apply config target invalid draft refused: true
SQLUI persistence settings draft probe apply config target invalid draft did not mutate: true
SQLUI persistence settings draft probe apply config target unsafe path rejected: true
SQLUI persistence settings draft probe apply config target runtime policy unchanged: true
SQLUI persistence settings draft probe apply config target SQLite did not create DB: true
SQLUI persistence settings draft probe apply config target no lifecycle side effects: true
SQLUI persistence settings draft probe apply config target artifact cleaned: true
SQLUI persistence settings draft probe apply config target policy default runtime unavailable: true
SQLUI persistence settings draft probe apply config target policy default cannot write: true
SQLUI persistence settings draft probe apply config target policy smoke-owned resolved: true
SQLUI persistence settings draft probe apply config target policy smoke-owned can write: true
SQLUI persistence settings draft probe apply config target policy future real target unavailable: true
SQLUI persistence settings draft probe apply config target policy documented strategy resolved: true
SQLUI persistence settings draft probe apply config target policy documented strategy cannot write: true
SQLUI persistence settings draft probe apply config target policy documented strategy no writable path: true
SQLUI persistence settings draft probe apply config target policy selected production descriptor resolved: true
SQLUI persistence settings draft probe apply config target policy selected production path described: true
SQLUI persistence settings draft probe apply config target policy selected production descriptor cannot write: true
SQLUI persistence settings draft probe apply config target policy selected production descriptor not project config: true
SQLUI persistence settings draft probe apply config target policy selected production target not created: true
SQLUI persistence settings draft probe apply config target policy guarded production descriptor blocked: true
SQLUI persistence settings draft probe apply config target policy documented strategy deterministic: true
SQLUI persistence settings draft probe apply config target policy production apply disabled: true
SQLUI persistence settings draft probe apply config target policy repeated deterministic: true
SQLUI persistence settings draft probe apply config target policy production enablement without request blocked: true
SQLUI persistence settings draft probe apply config target policy production enablement request recorded: true
SQLUI persistence settings draft probe apply config target policy production enablement blocked: true
SQLUI persistence settings draft probe apply config target policy production enablement no writable path: true
SQLUI persistence settings draft probe apply config target policy production enablement no side effects: true
SQLUI persistence settings draft probe apply config target policy production enablement deterministic: true
SQLUI persistence settings draft probe backend-only production apply unavailable without request: true
SQLUI persistence settings draft probe backend-only production apply created target: true
SQLUI persistence settings draft probe backend-only production apply wrote backend only: true
SQLUI persistence settings draft probe backend-only production apply did not write SQLite path: true
SQLUI persistence settings draft probe backend-only production apply did not write provider auto-init: true
SQLUI persistence settings draft probe backend-only production apply SQLite did not create DB: true
SQLUI persistence settings draft probe backend-only production apply no lifecycle side effects: true
SQLUI persistence settings draft probe backend-only production apply preserved config files: true
SQLUI persistence settings draft probe backend-only production apply no-change no-op: true
SQLUI persistence settings draft probe backend-only production apply provider auto-init rejected: true
SQLUI persistence settings draft probe backend-only production apply invalid draft refused: true
SQLUI persistence settings draft probe backend-only production apply invalid did not mutate: true
SQLUI persistence settings draft probe backend-only production apply artifact cleaned: true
SQLUI persistence settings draft probe backend-only production apply directory cleaned: true
SQLUI persistence settings draft probe backend-only production read absent no override: true
SQLUI persistence settings draft probe backend-only production read absent did not create target: true
SQLUI persistence settings draft probe backend-only production read after write succeeded: true
SQLUI persistence settings draft probe backend-only production read backend matched: true
SQLUI persistence settings draft probe backend-only production read no runtime side effects: true
SQLUI persistence settings draft probe backend-only production read unknown backend rejected: true
SQLUI persistence settings draft probe backend-only production read malformed rejected: true
SQLUI persistence settings draft probe backend-only production read artifact cleaned: true
SQLUI persistence settings draft probe backend-only production read directory cleaned: true
...
SQLUI persistence settings draft probe sidecar preserved during apply result adapter: true
...
For the backend-only runtime settings readback checkpoint, the expected coverage is:

- Absent `RuntimeSettings.ini` reads as no override and does not create the file or parent directory.
- Readback after the backend-only write succeeds and returns the expected backend value.
- Readback does not apply runtime settings, initialize providers/repositories, create database files, open SQLite for writing, run migrations, copy seeds, delete files, or read SQLite path/provider auto-init settings.
- Unknown backend and missing-backend files are rejected with validation/error status and without mutation.
- Repo `Config` and generated `Saved\Config` snapshots remain unchanged.
- Cleanup removes probe-created valid and invalid `RuntimeSettings.ini` artifacts plus the empty `PersistenceSettings` directory.

After the probe succeeds, database files under `Saved\SQLUI\SmokeTests\PersistenceSettingsDraft` should not exist, the temporary apply config target directory under `Saved\SQLUI\SmokeTests\PersistenceSettingsDraft\ApplyConfigTarget` should be gone, and the backend-only production apply/readback target `Saved\SQLUI\PersistenceSettings\RuntimeSettings.ini` plus its parent directory should be gone if the probe created them. A full checkpoint cleanup check should also confirm that the related read-only status paths used by this phase, such as `Saved\SQLUI\SmokeTests\PersistenceStatusSampleSurface` and `Saved\SQLUI\SmokeTests\PersistenceStatusDisplayRows`, contain no smoke-owned `.db`, `.db-journal`, `.db-wal`, or `.db-shm` files.
