...
The mutating settings editing and reset phase is planned in `docs/sqlui_persistence_settings_editing_reset_plan.md`. The first implementation slices add `-UsePersistenceSettingsDraftProbe` for non-mutating draft/pending settings validation, UI-safe validation display rows, an optional SQLUISamples sample/dev adapter that consumes those rows, and an optional C++ UMG widget shell contract for future Blueprint binding. The safe binding recipe for that draft widget shell is documented in `docs/sqlui_persistence_settings_draft_umg_usage.md`. Future implementation PRs that add apply behavior, backend selector UI, SQLite path editor UI, provider auto-init controls, or reset/delete actions should add focused smoke coverage for those behaviors while preserving the existing read-only status and draft-validation semantics.
...
This path proves validation-only draft behavior, UI-safe validation display-row formatting, SQLUISamples sample adapter consumption of those rows, and the optional C++ UMG widget shell binding contract. The future Blueprint/UMG binding recipe for this shell lives in [`sqlui_persistence_settings_draft_umg_usage.md`](sqlui_persistence_settings_draft_umg_usage.md). It does not apply settings, write config, add settings editing UI controls, add backend selector controls, add SQLite path editor controls, add provider auto-init toggle controls, add reset/delete behavior, add widget blueprint assets, add visual layout, attach widgets to the viewport, initialize providers or repositories, create database files, create directories from adapter/display/widget generation, open databases for writing, run migrations, copy seed databases, delete files outside smoke-owned cleanup, change startup behavior, edit Content or maps, or leave persistent database files behind. The only directory/file creation in this smoke path is the explicit smoke-owned sidecar fixture used to verify cleanup and non-deletion behavior.

After the probe succeeds, database files under `Saved\SQLUI\SmokeTests\PersistenceSettingsDraft` should not exist.
