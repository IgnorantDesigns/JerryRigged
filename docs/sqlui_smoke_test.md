...
The production config target strategy resolution adds no new smoke flag. Existing probes already confirm smoke-owned target behavior, target policy behavior, and the documented strategy's non-writable/no-path status. The guarded production target enablement request is covered by the same draft probe: an explicit request is recorded, but remains blocked, non-writable, pathless, deterministic, and side-effect free because the selected `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` target is design-only and not implemented. A future PR that enables that target must add explicit new or extended smoke assertions for target path resolution, smoke-owned versus real-target separation, committed `Config` preservation, generated `Saved/Config` diff/restore behavior if relevant, valid backend-only writes, invalid-draft refusal, no-change/no-op behavior, no DB creation, no DB write-open, no migrations, no seed copy, no provider/repository lifecycle work, and cleanup of any smoke-owned or real-target test artifacts.

The PR #138 production config target resolution checkpoint and PR #139 guarded production target enablement checkpoint use the existing `-UsePersistenceSettingsDraftProbe` path and add no new smoke flag. The probe covers the validation-only draft model, display rows, dry-run apply preview, apply/cancel contract, apply entrypoint skeleton refusal/no-mutation behavior, apply-result display/adapter/widget paths, smoke-owned apply config target scaffold, apply config target policy/resolver skeleton, documented production config target resolution, and guarded production target enablement request behavior. It verifies default/runtime Apply remains unavailable/not implemented, the smoke-owned target remains explicit and isolated, the documented future real project/user target remains unavailable/non-writable with no writable path, explicit production target enablement requests are recorded but blocked/non-writable/pathless, repo `Config` / `Saved/Config` snapshots remain unchanged, cleanup removes only smoke-owned files, no draft/status/apply probe DB or sidecar files remain, and no smoke-owned apply config target artifact remains. This docs-only design checkpoint changes the future target documentation only; it adds no new smoke command and keeps the existing probe expectations unchanged until an implementation PR writes the selected `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` target.
...
- Target policy reports default/runtime unavailable, explicit smoke-owned write-capable only for the smoke target, future real project/user target unavailable, documented production strategy non-writable with no path, and production Apply disabled.
- Guarded production target enablement request reports requested-but-blocked, non-writable, no path, no side effects, and deterministic output.
