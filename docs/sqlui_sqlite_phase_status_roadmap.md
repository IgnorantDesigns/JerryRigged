...
- A non-mutating apply config target policy/resolver skeleton now makes those target decisions explicit: default/runtime remains unavailable and non-writable, explicit smoke-owned targets remain the only write-capable targets, and a future project/user config target is represented as unavailable until a later PR enables and validates it.
...
| Persistence settings apply config target policy skeleton | Implemented | SQLUICore now has a non-mutating resolver that reports default/runtime target unavailable, smoke-owned target explicit-only/write-capable for smoke coverage, and future project/user target unavailable until a later PR scopes it. |
