...
- PR #139 codifies a guarded production target enablement request: explicit enablement can be requested and reported, but it remains blocked, non-writable, pathless, deterministic, and side-effect free because no concrete production target is selected yet.
...
The #137 production config target resolution slice represents that strategy in SQLUICore policy through `ResolveDocumentedProductionTargetStrategy()`. This is still target resolution only: the documented future real project/user target is identifiable in code, but it remains unavailable, non-writable, has no writable path, and production Apply remains disabled. The guarded enablement slice adds a request/result shape for explicit production target enablement, but it still reports the request as blocked and non-writable while no safe concrete target is selected. The smoke-owned target remains explicit-only and is still the only write-capable target. The draft probe verifies repo `Config` / `Saved/Config` preservation, no DB creation, no DB write-open, no migrations, no seed copy, no provider/repository lifecycle work, guarded enablement request/no-request behavior, and deterministic cleanup of smoke-owned artifacts.

The #139 checkpoint documents that guarded enablement is a blocked policy proof, not a real Apply enablement. It adds no concrete production target, no production target file/path, no real user/runtime config writes, no runtime settings application, no committed config changes, no provider/repository lifecycle behavior, no DB creation or write-open, no migrations, no seed copy, and no file deletion outside smoke-owned cleanup. The next implementation decision is still the exact safe target: a SQLUICore-owned `Saved/SQLUI` settings file, a plugin-managed `Saved/SQLUI` settings file, a separately justified user/project settings layer, or continued blocked behavior.
...
| Persistence settings guarded production target enablement | Implemented as blocked policy | #139 adds an explicit enablement request/resolution for the documented production target. Requests are recorded, but remain blocked, non-writable, pathless, deterministic, and side-effect free until a safe concrete production target is selected. |
