...
The recommended next persistence-settings slices remain ordered: keep backend-only write as the checkpointed base; add selected-target readback/resolution separately if needed; add provider auto-init write separately; add SQLite path write separately with path-safety and packaged-path validation; add backend selector UI only after SQLUICore write/readback behavior is tested; and require packaged validation before startup/package behavior consumes `RuntimeSettings.ini`.
...
- Implement any additional writes to the selected SQLUICore-owned `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` runtime settings file only in future scoped implementation PRs. Current code may describe that target and write only the backend value through the explicit guarded request, but must not create or write broader settings.
- Keep explicit smoke-owned targets as the only full-draft write-capable path, and keep the production target limited to backend-only writes.
...
| Persistence settings actual apply implementation gate and backend-only write | Implemented | SQLUICore has a callable `RequestPersistenceSettingsApply` path. Default requests report unavailable/not implemented, validation-blocked, no-change, or preview-only results without mutation. Explicit backend-only production requests can write just `Backend=<value>` to `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini`. Broader apply/save/config-write behavior, controls, lifecycle mutation, and database file behavior remain future work. |
...
| Persistence settings production config target descriptor and backend-only write | Implemented narrowly | SQLUICore can describe the selected target as `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini`, SQLUICore/plugin-managed, non-committed, not `DefaultEngine.ini`, not generated `Saved/Config`, not user/global editor settings, and not smoke-owned. Explicit backend-only production Apply may create/write that file with only a `Backend` value. The general resolver still exposes no broad writable production path, and no SQLite path/provider auto-init/migration/seed/reset/delete settings are written. |
