...
The production target decision is now documented before any real writes are enabled. #132 added the smoke-owned target scaffold, #133 documented that checkpoint, #134 added the target policy/resolver skeleton, and #135 documented that policy checkpoint. This strategy gate inventories candidate real targets and selects a SQLUICore-owned `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` design for future implementation, while production Apply stays closed because no code implements that target yet.
...
The #139 checkpoint documents that guarded enablement is a blocked policy proof, not a real Apply enablement. It adds no production target file/path, no real user/runtime config writes, no runtime settings application, no committed config changes, no provider/repository lifecycle behavior, no DB creation or write-open, no migrations, no seed copy, and no file deletion outside smoke-owned cleanup. The next implementation decision is no longer which target shape to use; it is whether and how to implement the selected SQLUICore-owned `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` target while preserving the blocked defaults and side-effect boundaries.
...
- Implement writes to the selected SQLUICore-owned `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` runtime settings file only in a future scoped implementation PR. Current code may describe that target, but must not create or write it.
