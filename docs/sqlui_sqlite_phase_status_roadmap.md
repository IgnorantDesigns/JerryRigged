...
| Persistence settings apply config target policy skeleton | Implemented | #134 adds a non-mutating resolver that reports default/runtime target unavailable, smoke-owned target explicit-only/write-capable for smoke coverage, and future project/user target unavailable until a later PR scopes it. |
