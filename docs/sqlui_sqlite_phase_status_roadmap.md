...
- `-UsePersistenceSettingsDraftProbe`: smoke coverage for the draft model, apply preview, apply/cancel contract, validation/apply-preview display rows, validation/apply-preview sample adapters, and validation/apply-preview C++ UMG widget shell contracts.
...
The dry-run apply-intent preview reports whether a future Apply would have changes, whether backend/SQLite/provider-auto-init policy would change, and whether restart/reopen/reinitialize messaging should be shown. The apply/cancel contract reports future-apply readiness, explicitly marks actual Apply execution unavailable/not implemented, and describes cancel/discard as value preview only. These surfaces use future-oriented "would change," "not implemented," and "not applied" semantics and do not write config or mutate runtime persistence.
...
- Draft validation, validation display generation, dry-run apply preview, apply/cancel contract generation, SQLUISamples draft adapter refresh, and the draft validation widget shell remain non-mutating: they do not create databases or directories, open databases for writing, run migrations, copy seeds, initialize providers/repositories, or delete files outside smoke-owned cleanup.
...
| Persistence settings draft validation/display rows, apply preview, apply/cancel contract, sample adapters, and UMG shells | `-UsePersistenceSettingsDraftProbe` | Covered | Verifies default/current draft validation, validation display rows, dry-run apply-preview no-change behavior, non-mutating apply/cancel contract behavior, apply-preview display rows, validation/apply-preview sample adapter consumption, validation/apply-preview C++ UMG widget shell reflection contracts, unknown backend rejection/error display/blocked preview/preview-display/blocked-contract error, SQLite draft/path display/preview/contract without DB creation, empty SQLite path rejection/error display/blocked preview/preview-display/blocked-contract error, provider auto-init pending validation/display/preview/preview-display/contract without policy mutation, deterministic validation/display/preview/preview-display/contract/adapter output, sidecar preservation, and cleanup. |
