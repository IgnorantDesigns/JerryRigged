...
- A focused validation/preview-only draft UMG usage guide now records the safe future widget blueprint subclass/binding recipe, display semantics, preview semantics, refresh/build boundaries, and manual local checklist for the draft validation/apply-preview widget shells.
...
| Persistence settings draft validation/display rows, apply preview, sample adapters, and UMG shells | `-UsePersistenceSettingsDraftProbe` | Covered | Verifies default/current draft validation, validation display rows, dry-run apply-preview no-change behavior, apply-preview display rows, validation/apply-preview sample adapter consumption, validation/apply-preview C++ UMG widget shell reflection contracts, unknown backend rejection/error display/blocked preview/preview-display error, SQLite draft/path display and preview without DB creation, empty SQLite path rejection/error display/blocked preview/preview-display error, provider auto-init pending validation/display/preview/preview-display without policy mutation, deterministic validation/display/preview/preview-display/adapter output, sidecar preservation, and cleanup. |
