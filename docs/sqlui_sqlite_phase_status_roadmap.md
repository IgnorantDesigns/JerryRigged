...
- An optional SQLUISamples persistence settings draft presenter now consumes the SQLUICore validation display rows for sample/dev-facing display without adding settings controls, apply/save behavior, DB creation, migrations, seed copy, provider/repository initialization, or destructive actions.
...
| Persistence settings draft validation/display rows and sample adapter | `-UsePersistenceSettingsDraftProbe` | Covered | Verifies default/current draft validation and display rows, sample adapter consumption, unknown backend rejection/error display, SQLite draft/path display without DB creation, empty SQLite path rejection/error display, provider auto-init pending validation/display without policy mutation, deterministic validation/display/adapter output, sidecar preservation, and cleanup. |
...
- `USQLUIPersistenceSettingsDraftLibrary` validates pending settings only, and `USQLUIPersistenceSettingsDraftDisplayLibrary` formats those validation results for UI consumption only. They do not write config, apply changes, initialize providers/repositories, create SQLite database files, run migrations, copy seeds, delete files, add UI controls, or change startup behavior.
...
1. Add the next settings draft helper slice: explicit Apply/Cancel request/result shapes that still avoid widget controls until the lifecycle policy is reviewed.
   Build on the completed validation-only draft model and display rows, keep SQLite opt-in, keep `InMemory` as the safe default, and do not write config or reinitialize providers until an explicitly scoped apply PR does so.
