...
- PR #134 adds an apply config target policy/resolver skeleton that makes those target decisions explicit: default/runtime remains unavailable and non-writable, explicit smoke-owned targets remain write-capable for smoke coverage, and the documented project/runtime config target is represented without exposing unsafe config locations.
- PR #135 documents the target policy checkpoint, and the production config target strategy gate documents the selected target before broad writes. The selected target design is a SQLUICore-owned `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` file. Unsafe targets are still rejected, and production/default Apply remains unavailable except for the later explicitly guarded backend-only value write.
- SQLUICore now codifies that documented strategy with `ResolveDocumentedProductionTargetStrategy()`: the documented future real project/user target is represented in code, carries a descriptor for `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini`, but remains non-writable and production Apply remains disabled.
- PR #139 codifies a guarded production target enablement request. Explicit enablement can be requested and reported as policy data, and the later backend-only apply slice uses the selected `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` target only for a single `Backend` value.
...
- A SQLUICore actual apply entrypoint now exists through `RequestPersistenceSettingsApply`. Default requests reuse the existing apply contract and return validation-blocked, no-change, or preview-only/unavailable result data without mutation. Explicit backend-only production requests can write only `Backend=<value>` to `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` while still performing no provider, repository, database, migration, seed-copy, reset/delete, startup, or SQLite path/provider-auto-init side effects.
- A backend-only apply write checkpoint now records the #143 selected-target scope: the selected target is `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini`, the write requires explicit guarded production enablement, default startup/status/descriptor/enablement resolution do not create the file, only `Backend=<value>` is serialized, smoke cleanup removes the probe-created artifact, and repo `Config` / generated `Saved\Config` stay unchanged.
- A backend-only runtime settings readback checkpoint now exists for that selected target. It is explicit and caller-invoked only, reads only `Backend`, treats an absent file as no runtime override, reports missing/unknown backend values as errors, does not apply the parsed backend to runtime/provider/repository state, does not read SQLite path/provider auto-init, and is covered by `-UsePersistenceSettingsDraftProbe`.
...
PR #134 adds `FSQLUIPersistenceSettingsApplyConfigTargetPolicy` as a SQLUICore-only resolver skeleton. It does not add broad production/user writes. It reports that the default/runtime target is unavailable, a safe explicit smoke-owned target is smoke/test-only and writable for smoke coverage, and the documented project/runtime config target is unavailable to the general resolver.

The documented production config target strategy is now also represented by `ResolveDocumentedProductionTargetStrategy()`. It intentionally does not pick `DefaultEngine.ini`, generated `Saved/Config`, user/global editor settings, or any other unsafe writable path. The older future project/user target resolver delegates to the documented strategy resolution so smoke coverage can prove the general resolver remains non-writable and deterministic.
...
- Default `RequestPersistenceSettingsApply` remains unavailable/not implemented unless the explicit backend-only production request is used.
- Real user/runtime config writes are limited to the backend-only `RuntimeSettings.ini` value.
...
The production target decision is now documented before broad writes are enabled. #132 added the smoke-owned target scaffold, #133 documented that checkpoint, #134 added the target policy/resolver skeleton, and #135 documented that policy checkpoint. This strategy gate inventories candidate real targets and selects a SQLUICore-owned `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` design. Production Apply remains closed for every setting except the explicit backend-only value write.

The #137 production config target resolution slice represents that strategy in SQLUICore policy through `ResolveDocumentedProductionTargetStrategy()`. The resolver still exposes no general writable production `ConfigFilePath` and does not create the file or directory by itself. The descriptor slice identifies `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini`, and the guarded enablement slice adds a request/result shape for explicit production target enablement. The backend-only apply slice is the first implementation that writes that selected target, and it writes only the backend value through `RequestPersistenceSettingsApply`. The smoke-owned target remains explicit-only for full draft serialization coverage. The draft probe verifies repo `Config` / `Saved/Config` preservation, no DB creation, no DB write-open, no migrations, no seed copy, no provider/repository lifecycle work, guarded enablement request/no-request behavior, backend-only production write/no-change/invalid/refusal behavior, and deterministic cleanup of smoke-owned and probe-created production artifacts.

The #139 checkpoint documents that guarded enablement is a policy proof, not a broad Apply enablement. By itself, it adds no production target file/path, no real user/runtime config writes, no runtime settings application, no committed config changes, no provider/repository lifecycle behavior, no DB creation or write-open, no migrations, no seed copy, and no file deletion outside smoke-owned cleanup. The follow-up backend-only apply slice is the first selected-target write: it writes only `Backend=<value>` to the SQLUICore-owned `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` target while preserving blocked defaults and side-effect boundaries. Any broader selected-target setting must still be scoped separately.
...
The dry-run apply-intent preview reports whether a future Apply would have changes, whether backend/SQLite/provider-auto-init policy would change, and whether restart/reopen/reinitialize messaging should be shown. The apply/cancel contract reports future-apply readiness for broad Apply, marks default Apply execution unavailable/not implemented, and describes cancel/discard as value preview only. The actual apply entrypoint reuses that contract for default requests and returns validation-blocked, no-change, or preview-only/unavailable result data without mutation; the explicit backend-only production request can write only `Backend=<value>` to `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini`, and the explicit readback helper can parse only that backend value without applying it. The apply/cancel contract display formatter turns that contract into UI-safe rows such as availability, actual apply implemented, restart/reopen/reinitialize requirements, cancel/discard preview, and validation/readiness messages. These surfaces use future-oriented "would change," "not implemented," "not applied," and backend-only result semantics and do not create DBs, run migrations, copy seeds, initialize providers/repositories, or mutate runtime persistence beyond the guarded backend setting file.
...
| Persistence settings apply/cancel contract | Implemented | SQLUICore reports future apply readiness and cancel/discard value preview through non-mutating contract helpers. Default Apply execution remains unavailable/not implemented; the only mutating apply slice is the explicit backend-only `RuntimeSettings.ini` write, and the only selected-target readback slice parses the backend value without applying it. No provider/repository lifecycle changes, DB creation, migrations, seed copy, destructive actions, UI controls, assets, maps, or startup wiring are added. |
...
| Persistence settings actual apply implementation gate, backend-only write, and backend-only readback | Implemented | SQLUICore has a callable `RequestPersistenceSettingsApply` path. Default requests report unavailable/not implemented, validation-blocked, no-change, or preview-only results without mutation. Explicit backend-only production requests can write just `Backend=<value>` to `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini`, and explicit readback can parse just that backend value while treating absence as no override and rejecting invalid backend content. Broader apply/save/config-write/readback behavior, controls, lifecycle mutation, startup consumption, and database file behavior remain future work. |
...
| Persistence settings apply config target policy and documented production strategy resolution | Implemented | #134 adds a resolver that reports default/runtime target unavailable, smoke-owned target explicit-only/write-capable for smoke coverage, and the documented project/runtime target unavailable to the general resolver. The documented production target strategy is resolved in SQLUICore without exposing `DefaultEngine.ini`, generated `Saved/Config`, user/global editor settings, or a broad writable production path. |
| Persistence settings guarded production target enablement | Implemented as blocked policy | #139 adds an explicit enablement request/resolution for the documented production target. Requests are recorded, but remain blocked, non-writable, deterministic, and side-effect free until the selected production target is implemented and validated. |
| Persistence settings production config target descriptor and backend-only write/readback | Implemented narrowly | SQLUICore can describe the selected target as `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini`, SQLUICore/plugin-managed, non-committed, not `DefaultEngine.ini`, not generated `Saved/Config`, not user/global editor settings, and not smoke-owned. Explicit backend-only production Apply may create/write that file with only a `Backend` value, and explicit backend-only readback may parse only that value. The general resolver still exposes no broad writable production path, no startup consumer exists, and no SQLite path/provider auto-init/migration/seed/reset/delete settings are written or read. |
