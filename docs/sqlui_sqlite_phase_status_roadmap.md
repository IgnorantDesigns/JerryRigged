...
- PR #139 codifies a guarded production target enablement request: explicit enablement can be requested and reported, but it remains blocked, non-writable, pathless, deterministic, and side-effect free because the selected `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` target is not implemented yet.
...
The #137 production config target resolution slice represents that strategy in SQLUICore policy through `ResolveDocumentedProductionTargetStrategy()`. This is still target resolution only: the documented future real project/user target is identifiable in code, but it remains unavailable, non-writable, has no writable path, and production Apply remains disabled. The guarded enablement slice adds a request/result shape for explicit production target enablement, but it still reports the request as blocked and non-writable while the selected target is design-only and unimplemented. The smoke-owned target remains explicit-only and is still the only write-capable target. The draft probe verifies repo `Config` / `Saved/Config` preservation, no DB creation, no DB write-open, no migrations, no seed copy, no provider/repository lifecycle work, guarded enablement request/no-request behavior, and deterministic cleanup of smoke-owned artifacts.
...
- Implement the selected SQLUICore-owned `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` runtime settings file only in a future scoped implementation PR.
...
| Persistence settings guarded production target enablement | Implemented as blocked policy | #139 adds an explicit enablement request/resolution for the documented production target. Requests are recorded, but remain blocked, non-writable, pathless, deterministic, and side-effect free until the selected production target is implemented and validated. |
| Persistence settings production config target design | Documented | The selected future target is a SQLUICore-owned `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` settings file for validated persistence settings intent only. No code writes it yet; production/default Apply remains unavailable, and smoke-owned targets remain the only write-capable path. |
