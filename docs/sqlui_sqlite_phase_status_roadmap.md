...
| Persistence settings draft validation/display rows, apply preview, apply/cancel contract, apply result display, sample adapters, and UMG shells | `-UsePersistenceSettingsDraftProbe` | Covered | Verifies default/current draft validation, validation display rows, dry-run apply-preview no-change behavior, non-mutating apply/cancel contract behavior, unavailable/non-mutating apply request behavior, apply-result display rows, apply-preview display rows, apply/cancel contract display rows, validation/apply-preview/apply-contract sample adapter consumption, validation/apply-preview/apply-contract C++ UMG widget shell reflection contracts, unknown backend rejection/error display/blocked preview/preview-display/blocked-contract/contract-display/apply-result-display error, SQLite draft/path display/preview/contract/contract-display/apply-result-display without DB creation, empty SQLite path rejection/error display/blocked preview/preview-display/blocked-contract/contract-display error, provider auto-init pending validation/display/preview/preview-display/contract/contract-display/apply-result-display without policy mutation, deterministic validation/display/preview/preview-display/contract-display/apply-result-display/contract/adapter output, sidecar preservation, config preservation, and cleanup. |
...
1. Add the next settings draft helper slice: mutating Apply execution behavior behind the existing SQLUICore request/result skeleton and documented implementation gate.
   Build on the completed validation-only draft model, dry-run apply-intent preview, non-mutating apply/cancel contract, apply-result display rows, and display rows, keep SQLite opt-in, keep `InMemory` as the safe default, refuse invalid drafts without mutation, and do not write config or reinitialize providers until an explicitly scoped and smoke-tested apply PR does so.
