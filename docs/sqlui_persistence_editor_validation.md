...
- The production config target strategy gate in [`sqlui_persistence_config_target_strategy.md`](sqlui_persistence_config_target_strategy.md) is now represented by a non-writable SQLUICore policy resolution. It records candidate and rejected real targets before implementation, keeps production Apply unavailable, and does not claim manual editor, PIE, or packaged validation for real config writes.
- The production config target resolution checkpoint is documentation-only. It does not claim additional manual editor validation beyond the existing null-RHI commandlet/editor-launch checks, and manual editor inspection remains recommended before committing any real settings controls, production config-write behavior, startup wiring, maps, assets, or provider lifecycle changes.
- PR #139's guarded production target enablement request is covered by commandlet smoke only. It records and blocks an explicit enablement request, keeps the resolver result non-writable, and does not claim additional manual editor, PIE, or packaged validation because no settings control, runtime config write, startup wiring, provider lifecycle, DB creation/write-open, migration, seed copy, or file deletion behavior was added.
- The production config target descriptor identifies `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` as the SQLUICore-owned target, but descriptor resolution does not create the file or directory, add controls, or change runtime behavior. The later backend-only writer and reader are separate and commandlet-smoke-covered; future broader writes/readback should add commandlet smoke coverage and, if startup/default-map/provider lifecycle or packaged runtime behavior can be affected, packaged validation before claiming production readiness.
- PR #143's backend-only production apply write is covered by commandlet smoke only. It can write only `Backend=<value>` to `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` through an explicit guarded request, then smoke cleanup removes the probe-created artifact. This does not claim manual editor, PIE, or packaged validation because no settings controls, widget Blueprint assets, maps, startup wiring, provider lifecycle, DB creation/write-open, migrations, seed copy, SQLite path/provider auto-init writes, reset/delete behavior, or runtime config consumption was added.
- PR #145's backend-only runtime settings readback checkpoint is also covered by commandlet smoke only. It reads only `Backend` from `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` when explicitly called, treats absence as no override, rejects missing/unknown backend values, and does not apply runtime settings, initialize providers/repositories, create DBs, run migrations, copy seeds, delete files, add controls, or change startup behavior. This docs-only checkpoint adds no manual editor or PIE claim; future startup/runtime consumption of that file needs its own editor/PIE/packaged validation scope.
