...
- PR #139's guarded production target enablement request is covered by commandlet smoke only. It records and blocks an explicit enablement request, keeps the result non-writable, and does not claim additional manual editor, PIE, or packaged validation because no concrete target implementation, settings control, runtime config write, startup wiring, provider lifecycle, DB creation/write-open, migration, seed copy, or file deletion behavior was added.
- The production config target descriptor identifies `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` as the future SQLUICore-owned target, but it does not implement that target, enable writes, create the file or directory, add controls, or change runtime behavior. Future implementation of that target should add commandlet smoke coverage and, if startup/default-map/provider lifecycle or packaged runtime behavior can be affected, packaged validation before claiming production readiness.
