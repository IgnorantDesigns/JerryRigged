...
- The backend-only runtime settings readback checkpoint is also covered by commandlet smoke only. It reads only `Backend` from `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` when explicitly called, treats absence as no override, rejects missing/unknown backend values, and does not apply runtime settings, initialize providers/repositories, create DBs, run migrations, copy seeds, delete files, add controls, or change startup behavior. Future startup/runtime consumption of that file needs its own editor/PIE/packaged validation scope.
