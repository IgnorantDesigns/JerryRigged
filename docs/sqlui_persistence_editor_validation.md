...
- The apply config target policy/resolver skeleton is covered by the same commandlet draft probe only. It makes default/runtime, smoke-owned, and future project/user target decisions explicit, but it does not add a manual editor flow, a real Apply button, or production/user config writes.
