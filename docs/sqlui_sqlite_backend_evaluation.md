...
- The #143 backend-only checkpoint is not SQLite runtime enablement. Writing `Backend=SQLite` records only the selected backend value in `RuntimeSettings.ini`; it does not create a SQLite database, create database directories, initialize providers/repositories, run migrations, copy seeds, write SQLite path/provider auto-init, or change startup behavior. Runtime readback of that file and any lifecycle behavior remain future work.
