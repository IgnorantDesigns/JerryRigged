...
- The #143 backend-only checkpoint and follow-up readback are not SQLite runtime enablement. Writing `Backend=SQLite` records only the selected backend value in `RuntimeSettings.ini`, and explicit readback parses only that backend value. Neither path creates a SQLite database, creates database directories, initializes providers/repositories, runs migrations, copies seeds, writes or reads SQLite path/provider auto-init, applies runtime settings, or changes startup behavior. Startup consumption and lifecycle behavior remain future work.
