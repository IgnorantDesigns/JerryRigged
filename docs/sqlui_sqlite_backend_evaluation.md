...
- Production/user-facing runtime settings UI/UX, actual settings apply/save/config-write behavior, normal startup initialization policy, and database path policy beyond the safe default-off config-backed settings/provider/subsystem path, docs-only settings editing/reset plan, non-mutating draft validation/apply-preview/apply-contract/apply-result display model, sample/dev display/widget-shell contracts including the apply-result presenter and C++ shell, validation-only draft widget-shell usage guide, dry-run apply-preview widget-shell usage guide, apply/cancel contract widget-shell usage guide, apply-result widget-shell usage guide, #111 docs-only draft validation foundation checkpoint, #117 docs-only apply-preview UI foundation checkpoint, the #124 docs-only apply/cancel contract UI foundation checkpoint, and the docs-only actual apply implementation gate.
