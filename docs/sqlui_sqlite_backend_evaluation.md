...
This document records the SQLite backend evaluation that led SQLUI to use engine `SQLiteCore` as the active runtime candidate. The original backend evaluation was documentation-only; the current code now includes SQLiteCore wiring, repository operations, factory selection, an explicit runtime configuration resolver, an explicit runtime integration helper above resolver/seed-copy/factory creation, a storage-agnostic runtime repository provider, config-backed runtime repository settings with safe defaults, a passive runtime provider `GameInstance` subsystem, a persistence settings draft/validation model plus dry-run apply-intent preview and apply/cancel contract, validation/apply-preview/apply-contract/apply-result display rows, sample/dev adapters for validation, apply-preview, apply/cancel contract, and apply-result rows, optional C++ UMG widget shell contracts for validation/apply-preview/apply-contract/apply-result display, docs-only safe usage guides for the draft validation, apply-preview, apply/cancel contract, and apply-result widget shells, the #111 docs-only draft validation foundation checkpoint, the #117 docs-only apply-preview UI foundation checkpoint, the #124 docs-only apply/cancel contract UI foundation checkpoint, an actual apply implementation gate, a smoke-owned apply config target scaffold, the #134 apply config target policy/resolver skeleton, a production config target strategy resolution, a selected production target descriptor for `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini`, a guarded production target enablement request, a backend-only selected-target write, explicit backend-only selected-target readback, an explicit seed database copy policy helper, opt-in schema initialization, a migration version/status helper for the known layout schema migration set, serialized opt-in async callback execution with queue shutdown/stale-callback suppression for `LoadLayout` and `SaveLayout`, local smoke coverage, a local packaged-build validation scaffold, an opt-in packaged runtime SQLite lifecycle smoke, an opt-in packaged runtime provider startup smoke, and an opt-in packaged runtime provider subsystem smoke. SQLite is still not the default backend, and this work still does not add migrations inside the factory, broad user/runtime settings apply/save/config-write behavior, settings editing UI controls, maps, assets, CI, source-controlled seed databases, or persistent database files.
...
For the production config target strategy gate, broad non-writable policy resolution, and backend-only selected-target write, see [`sqlui_persistence_config_target_strategy.md`](sqlui_persistence_config_target_strategy.md).
...
For the dry-run apply-preview UMG binding recipe, see [`sqlui_persistence_settings_apply_preview_umg_usage.md`](sqlui_persistence_settings_apply_preview_umg_usage.md).
For the apply/cancel contract UMG binding recipe, see [`sqlui_persistence_settings_apply_contract_umg_usage.md`](sqlui_persistence_settings_apply_contract_umg_usage.md).
For the apply-result UMG binding recipe, see [`sqlui_persistence_settings_apply_result_umg_usage.md`](sqlui_persistence_settings_apply_result_umg_usage.md).
...
- Do not treat the smoke-owned apply config target scaffold as production/user runtime settings Apply.
- Do not treat the production config target strategy gate, its policy resolver, or the guarded enablement request as enabling production/user runtime settings Apply.
...
- Production/user-facing runtime settings UI/UX, broad settings apply/save/config-write behavior, normal startup initialization policy, and database path policy beyond the safe default-off config-backed settings/provider/subsystem path. The completed foundation includes the docs-only settings editing/reset plan, draft validation/apply-preview/apply-contract/apply-result display model, sample/dev display/widget-shell contracts including the apply-result presenter and C++ shell, validation-only draft widget-shell usage guide, dry-run apply-preview widget-shell usage guide, apply/cancel contract widget-shell usage guide, apply-result widget-shell usage guide, #111 docs-only draft validation foundation checkpoint, #117 docs-only apply-preview UI foundation checkpoint, the #124 docs-only apply/cancel contract UI foundation checkpoint, the #125 actual apply implementation gate, the completed #105-#131 apply entrypoint/result UI foundation checkpoint, the first smoke-owned apply config target scaffold, the #134 apply config target policy/resolver skeleton, and the production config target strategy gate. The scaffold writes only an explicit temporary `Saved/SQLUI/SmokeTests` target for smoke coverage; the selected production target is `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini`; the guarded backend-only production apply path writes only `Backend=<value>` when explicitly requested; and explicit readback parses only that backend value without applying it. Together they prove isolated target mechanics, invalid-draft refusal, repo config preservation, cleanup, backend-only selected-target writing, and backend-only selected-target readback, but they do not imply broad runtime settings application, SQLite path/provider auto-init production writes/readback, provider lifecycle work, database work, startup consumption, or a SQLite-by-default policy.
- The production config target resolution, descriptor, guarded enablement, backend-only write, and backend-only readback checkpoints resolve only the first selected-target storage/read slice. Remaining work is to expand writes/readback beyond backend-only, keep smoke-owned and selected targets separate, preserve config diff/snapshot coverage, and preserve no DB/lifecycle side effects before any settings UI controls or runtime config loading are enabled.
- The #139 guarded enablement request makes production-target intent explicit and testable, and the backend-only request now validates the selected SQLUICore-owned `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` storage surface for persistence settings intent only. `DefaultEngine.ini`, committed defaults, user/global editor settings, implicit `Saved/Config`, and widget-owned writes remain rejected for runtime Apply.
- The #143 backend-only checkpoint and PR #145 readback are not SQLite runtime enablement. Writing `Backend=SQLite` records only the selected backend value in `RuntimeSettings.ini`, and explicit readback parses only that backend value. Neither path creates a SQLite database, creates database directories, initializes providers/repositories, runs migrations, copies seeds, writes or reads SQLite path/provider auto-init, applies runtime settings, or changes startup behavior. Broader merge/preview reporting, startup consumption, provider auto-init, SQLite path policy, and lifecycle behavior remain future work.
