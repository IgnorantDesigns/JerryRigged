...
The dry-run apply-intent preview, apply-preview display rows, and optional SQLUISamples apply-preview presenter report what a future Apply would do, including whether a draft is valid enough to apply, whether it has changes, whether backend/SQLite/provider-auto-init policy would change, and whether restart/reopen/reinitialize messaging should be shown. They use "would change" and "not applied" semantics and do not write config, create directories or database files, open databases for writing, run migrations, copy seed databases, initialize providers/repositories, or delete files except smoke-owned cleanup in tests.
...
The draft settings model, apply-intent preview, apply-preview display rows, validation display rows, sample adapters, and C++ widget shell may be used by future controls, but adding them does not add those controls. UI surfaces should keep treating draft validation and apply preview as dry-run/pending-state steps until a later PR adds explicit Apply/Cancel behavior.
