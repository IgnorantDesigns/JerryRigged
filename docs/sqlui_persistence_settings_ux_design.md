...
The dry-run apply-intent preview, apply/cancel contract, actual apply entrypoint skeleton, apply-preview display rows, apply/cancel contract display rows, apply-result display rows, optional SQLUISamples apply-preview presenter, optional SQLUISamples apply/cancel contract presenter, optional SQLUISamples apply-result presenter, optional apply-preview widget shell, and optional apply/cancel contract widget shell report what a future Apply would do, including whether a draft is valid enough to apply, whether it has changes, whether backend/SQLite/provider-auto-init policy would change, whether restart/reopen/reinitialize messaging should be shown, and that actual Apply execution is still unavailable/not implemented. The apply entrypoint skeleton returns validation-blocked, no-change, or preview-only/unavailable results and keeps every config/settings/provider/repository/database/file side-effect flag false. The apply-result display rows and sample presenter format those result flags as safe user-readable statements such as "Config not written," "Settings not changed," "Provider not initialized," "Database files not created," and "Database not opened for writing." Cancel/discard is represented as value-preview data only. The safe binding recipes are documented in [`sqlui_persistence_settings_apply_preview_umg_usage.md`](sqlui_persistence_settings_apply_preview_umg_usage.md) for the apply-preview shell and [`sqlui_persistence_settings_apply_contract_umg_usage.md`](sqlui_persistence_settings_apply_contract_umg_usage.md) for the apply/cancel contract shell. They use "would change," "not implemented," "not applied," and "not saved" semantics and do not write config, create directories or database files, open databases for writing, run migrations, copy seed databases, initialize providers/repositories, or delete files except smoke-owned cleanup in tests.
