...
The #139 guarded enablement checkpoint is therefore a policy proof, not a UI readiness signal. It proves an explicit production target enablement request can be recorded and blocked while no concrete target exists. The product UI must continue to present Apply/Save/config-write behavior as unavailable until a later SQLUICore PR chooses an exact safe target, keeps smoke-owned and production paths separate, and validates invalid/no-change/write paths without DB, provider, repository, migration, seed-copy, startup, or default-policy side effects.
