...
The first settings-editing foundation slice now exists as `FSQLUIPersistenceSettingsDraft`, `FSQLUIPersistenceSettingsValidationResult`, `FSQLUIPersistenceSettingsApplyPreviewResult`, `FSQLUIPersistenceSettingsApplyContractResult`, `FSQLUIPersistenceSettingsCancelPreviewResult`, and `USQLUIPersistenceSettingsDraftLibrary`. It is still non-mutating: callers can represent current and pending runtime settings, validate backend/path/provider-auto-init choices, reset the in-memory draft value back to current values, build a dry-run apply-intent preview, query non-executing future-apply readiness, and describe cancel/discard as a pure value preview, but no settings are applied or saved. Draft validation, apply-preview generation, and apply/cancel contract generation do not create database files or directories, open databases for writing, run migrations, copy seed databases, initialize providers/repositories, delete files, add UI controls, or change startup behavior. `USQLUIPersistenceSettingsDraftDisplayLibrary` now adds a UI-safe summary/row formatter for validation results so a future sample or settings panel can show pending backend/path/provider-auto-init status without owning apply, save, or lifecycle behavior.
...
The dry-run apply-intent preview, apply/cancel contract, apply-preview display rows, optional SQLUISamples apply-preview presenter, and optional apply-preview widget shell report what a future Apply would do, including whether a draft is valid enough to apply, whether it has changes, whether backend/SQLite/provider-auto-init policy would change, whether restart/reopen/reinitialize messaging should be shown, and that actual Apply execution is still unavailable/not implemented. Cancel/discard is represented as value-preview data only. The safe binding recipe for that apply-preview shell is documented in [`sqlui_persistence_settings_apply_preview_umg_usage.md`](sqlui_persistence_settings_apply_preview_umg_usage.md). They use "would change," "not implemented," and "not applied" semantics and do not write config, create directories or database files, open databases for writing, run migrations, copy seed databases, initialize providers/repositories, or delete files except smoke-owned cleanup in tests.

The follow-up #117 apply-preview foundation checkpoint declares the #112-#116 dry-run apply-preview UI chain complete. That checkpoint is docs-only, and the later non-mutating apply/cancel contract still does not add actual mutating Apply/Cancel execution, config writes, settings editing controls, backend selector controls, SQLite path editing, provider auto-init controls, reset/delete behavior, widget blueprint assets, maps, startup wiring, viewport attachment, polling, ticking, auto-refresh, migrations, seed-copy behavior, provider/repository initialization, or default policy changes.
