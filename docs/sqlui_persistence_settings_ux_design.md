...
The first settings-editing foundation slice now exists as `FSQLUIPersistenceSettingsDraft`, `FSQLUIPersistenceSettingsValidationResult`, `FSQLUIPersistenceSettingsApplyPreviewResult`, `FSQLUIPersistenceSettingsApplyContractResult`, `FSQLUIPersistenceSettingsCancelPreviewResult`, and `USQLUIPersistenceSettingsDraftLibrary`. It is still non-mutating: callers can represent current and pending runtime settings, validate backend/path/provider-auto-init choices, reset the in-memory draft value back to current values, build a dry-run apply-intent preview, query non-executing future-apply readiness, and describe cancel/discard as a pure value preview, but no settings are applied or saved. Draft validation, apply-preview generation, apply/cancel contract generation, and their display-row formatters do not create database files or directories, open databases for writing, run migrations, copy seed databases, initialize providers/repositories, delete files, add UI controls, or change startup behavior. `USQLUIPersistenceSettingsDraftDisplayLibrary`, `USQLUIPersistenceSettingsApplyPreviewDisplayLibrary`, and `USQLUIPersistenceSettingsApplyContractDisplayLibrary` add UI-safe summary/row formatters for validation, apply-preview, and apply/cancel contract results so a future sample or settings panel can show pending backend/path/provider-auto-init status, future apply readiness, and cancel/discard value preview without owning apply, save, or lifecycle behavior.
...
The non-mutating persistence settings draft validation UI foundation is complete as a safe base for future Apply/Cancel work. It includes the #105 editing/reset UX plan, #106 validation-only draft model, #107 validation display rows/summary, #108 optional SQLUISamples draft validation presenter/adapter, #109 optional C++ UMG widget shell, #110 safe UMG usage guide, #111 final checkpoint, the SQLUICore dry-run apply-intent preview, the SQLUICore non-mutating apply/cancel contract, UI-safe apply-preview display rows/summary, UI-safe apply/cancel contract display rows/summary, optional SQLUISamples apply-preview and apply/cancel contract presenter/adapters, optional SQLUISamples apply-preview and apply/cancel contract C++ UMG widget shells, and `-UsePersistenceSettingsDraftProbe` smoke coverage.
...
The dry-run apply-intent preview, apply/cancel contract, apply-preview display rows, apply/cancel contract display rows, optional SQLUISamples apply-preview presenter, optional SQLUISamples apply/cancel contract presenter, optional apply-preview widget shell, and optional apply/cancel contract widget shell report what a future Apply would do, including whether a draft is valid enough to apply, whether it has changes, whether backend/SQLite/provider-auto-init policy would change, whether restart/reopen/reinitialize messaging should be shown, and that actual Apply execution is still unavailable/not implemented. Cancel/discard is represented as value-preview data only. The safe binding recipes are documented in [`sqlui_persistence_settings_apply_preview_umg_usage.md`](sqlui_persistence_settings_apply_preview_umg_usage.md) for the apply-preview shell and [`sqlui_persistence_settings_apply_contract_umg_usage.md`](sqlui_persistence_settings_apply_contract_umg_usage.md) for the apply/cancel contract shell. They use "would change," "not implemented," and "not applied" semantics and do not write config, create directories or database files, open databases for writing, run migrations, copy seed databases, initialize providers/repositories, or delete files except smoke-owned cleanup in tests.

The follow-up #117 apply-preview foundation checkpoint declares the #112-#116 dry-run apply-preview UI chain complete. The subsequent non-mutating apply/cancel contract and contract display rows still do not add actual mutating Apply/Cancel execution, config writes, settings editing controls, backend selector controls, SQLite path editing, provider auto-init controls, reset/delete behavior, widget blueprint assets, maps, startup wiring, viewport attachment, polling, ticking, auto-refresh, migrations, seed-copy behavior, provider/repository initialization, or default policy changes.
