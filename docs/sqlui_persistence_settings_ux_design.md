...
The first settings-editing foundation slice now exists as `FSQLUIPersistenceSettingsDraft`, `FSQLUIPersistenceSettingsValidationResult`, `FSQLUIPersistenceSettingsApplyPreviewResult`, `FSQLUIPersistenceSettingsApplyContractResult`, `FSQLUIPersistenceSettingsCancelPreviewResult`, `FSQLUIPersistenceSettingsApplyRequest`, `FSQLUIPersistenceSettingsApplyResult`, and `USQLUIPersistenceSettingsDraftLibrary`. It is still non-mutating: callers can represent current and pending runtime settings, validate backend/path/provider-auto-init choices, reset the in-memory draft value back to current values, build a dry-run apply-intent preview, query non-executing future-apply readiness, describe cancel/discard as a pure value preview, and call an actual apply entrypoint skeleton that returns unavailable/not implemented without mutation. Draft validation, apply-preview generation, apply/cancel contract generation, apply-request skeleton execution, and their display-row formatters do not apply settings, write config, create database files or directories, open databases for writing, run migrations, copy seed databases, initialize providers/repositories, delete files, add UI controls, or change startup behavior. `USQLUIPersistenceSettingsDraftDisplayLibrary`, `USQLUIPersistenceSettingsApplyPreviewDisplayLibrary`, and `USQLUIPersistenceSettingsApplyContractDisplayLibrary` add UI-safe summary/row formatters for validation, apply-preview, and apply/cancel contract results so a future sample or settings panel can show pending backend/path/provider-auto-init status, future apply readiness, and cancel/discard value preview without owning apply, save, or lifecycle behavior.
...
This checkpoint still does not implement a product settings screen. It adds no settings controls, backend selector UI, SQLite path editor UI, provider auto-init controls, actual settings apply/save/config-write behavior, reset/delete actions, widget blueprint assets, maps, startup wiring, viewport attachment, polling, ticking, auto-refresh, provider/repository initialization, migrations, seed-copy behavior, or default config changes. The draft validation, apply-preview, apply/cancel contract, and display paths remain caller-invoked, display/report-only, and non-mutating.

The dry-run apply-intent preview, apply/cancel contract, actual apply entrypoint skeleton, apply-preview display rows, apply/cancel contract display rows, optional SQLUISamples apply-preview presenter, optional SQLUISamples apply/cancel contract presenter, optional apply-preview widget shell, and optional apply/cancel contract widget shell report what a future Apply would do, including whether a draft is valid enough to apply, whether it has changes, whether backend/SQLite/provider-auto-init policy would change, whether restart/reopen/reinitialize messaging should be shown, and that actual Apply execution is still unavailable/not implemented. The apply entrypoint skeleton returns validation-blocked, no-change, or preview-only/unavailable results and keeps every config/settings/provider/repository/database/file side-effect flag false. Cancel/discard is represented as value-preview data only. The safe binding recipes are documented in [`sqlui_persistence_settings_apply_preview_umg_usage.md`](sqlui_persistence_settings_apply_preview_umg_usage.md) for the apply-preview shell and [`sqlui_persistence_settings_apply_contract_umg_usage.md`](sqlui_persistence_settings_apply_contract_umg_usage.md) for the apply/cancel contract shell. They use "would change," "not implemented," and "not applied" semantics and do not write config, create directories or database files, open databases for writing, run migrations, copy seed databases, initialize providers/repositories, or delete files except smoke-owned cleanup in tests.
