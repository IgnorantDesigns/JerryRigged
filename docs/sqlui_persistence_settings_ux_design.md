...
The backend and policy pieces now exist for a safe first UI. Tiny optional SQLUISamples sample/dev presenter, panel-adapter, and C++ UMG widget shell objects already provide Blueprint-callable, explicitly refreshed read-only display-row hooks for validation and Blueprint/sample consumption, and SQLUICore now has dry-run apply-intent preview plus apply/cancel contract and apply-result display rows for drafts. SQLUICore also has the #134 apply config target policy/resolver skeleton plus a documented production target strategy resolver, which keep default/runtime Apply unavailable unless the caller uses a separately guarded request path. The selected production target descriptor identifies `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` as SQLUICore/plugin-managed, non-committed, not `DefaultEngine.ini`, not generated `Saved/Config`, and not user/global editor settings. The first actual production write is intentionally backend-only: an explicit `RequestPersistenceSettingsApply` request may write just `Backend=<value>` to that target. The first readback is also backend-only and caller-invoked: SQLUICore can parse that backend value from the file, treat absence as no override, and report invalid backend content without applying it. SQLite path, provider auto-init, migration/seed/reset/delete settings, provider/repository lifecycle, DB creation/write-open, startup behavior, and UI controls remain out of scope. SQLUISamples also has sample/dev adapters and optional C++ UMG widget shells for validation, apply-preview, apply/cancel contract, and apply-result rows, but the full product settings UI is still future work. The broader apply/save/config-write implementation is gated in [`sqlui_persistence_settings_editing_reset_plan.md`](sqlui_persistence_settings_editing_reset_plan.md) and must be SQLUICore-first before real controls are introduced. This design keeps SQLite explicit, keeps `InMemory` as the safe default, and routes database status/reset behavior through SQLUICore helpers instead of widget-owned storage logic.
...
- [`sqlui_persistence_settings_draft_umg_usage.md`](sqlui_persistence_settings_draft_umg_usage.md) documents the validation-only draft settings UMG binding recipe for the optional validation widget shell.
- [`sqlui_persistence_settings_apply_preview_umg_usage.md`](sqlui_persistence_settings_apply_preview_umg_usage.md) documents the focused dry-run apply-preview UMG binding recipe for the optional apply-preview widget shell.
- [`sqlui_persistence_settings_apply_contract_umg_usage.md`](sqlui_persistence_settings_apply_contract_umg_usage.md) documents the focused apply/cancel contract UMG binding recipe for the optional contract widget shell.
- [`sqlui_persistence_settings_apply_result_umg_usage.md`](sqlui_persistence_settings_apply_result_umg_usage.md) documents the focused apply-result UMG binding recipe for the optional apply-result widget shell.
...
The first settings-editing foundation slice now exists as `FSQLUIPersistenceSettingsDraft`, `FSQLUIPersistenceSettingsValidationResult`, `FSQLUIPersistenceSettingsApplyPreviewResult`, `FSQLUIPersistenceSettingsApplyContractResult`, `FSQLUIPersistenceSettingsCancelPreviewResult`, `FSQLUIPersistenceSettingsApplyRequest`, `FSQLUIPersistenceSettingsApplyResult`, and `USQLUIPersistenceSettingsDraftLibrary`. Callers can represent current and pending runtime settings, validate backend/path/provider-auto-init choices, reset the in-memory draft value back to current values, build a dry-run apply-intent preview, query non-executing future-apply readiness, describe cancel/discard as a pure value preview, and call the apply entrypoint. Default apply requests remain unavailable/not implemented without mutation. The explicit backend-only production apply request is the single mutating exception and writes only `Backend=<value>` to `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini`. A separate explicit readback helper can parse only that backend value from the same file, but does not feed it into active runtime settings or provider state. Draft validation, apply-preview generation, apply/cancel contract generation, default apply requests, backend-only readback, apply-result display generation, and their display-row formatters do not create database files, open databases for writing, run migrations, copy seed databases, initialize providers/repositories, delete files, add UI controls, or change startup behavior. `USQLUIPersistenceSettingsDraftDisplayLibrary`, `USQLUIPersistenceSettingsApplyPreviewDisplayLibrary`, `USQLUIPersistenceSettingsApplyContractDisplayLibrary`, and `USQLUIPersistenceSettingsApplyResultDisplayLibrary` add UI-safe summary/row formatters for validation, apply-preview, apply/cancel contract, and apply result output so a future sample or settings panel can show pending backend/path/provider-auto-init status, future apply readiness, explicit unavailable/not-implemented or backend-only apply results, and cancel/discard value preview without owning apply, save, or lifecycle behavior.

`USQLUISamplePersistenceSettingsDraftPresenter`, `USQLUISamplePersistenceSettingsApplyPreviewPresenter`, `USQLUISamplePersistenceSettingsApplyContractPresenter`, `USQLUISamplePersistenceSettingsApplyResultPresenter`, `USQLUISamplePersistenceSettingsDraftPanelWidget`, `USQLUISamplePersistenceSettingsApplyPreviewPanelWidget`, `USQLUISamplePersistenceSettingsApplyContractPanelWidget`, and `USQLUISamplePersistenceSettingsApplyResultPanelWidget` are optional SQLUISamples sample/dev surfaces for those draft validation, apply-preview, apply/cancel contract, and apply-result rows. The widget shells are C++ `UUserWidget` subclasses only: they create no visual layout, add no widget blueprint asset, are not added to the viewport, are not wired into maps/startup/config, and do not refresh from lifecycle hooks. Their caller-invoked refresh/build functions delegate to presenters, cache rows/result/summary in memory, and remain validation/preview/display-only. The apply-result shell consumes the SQLUICore apply entrypoint/display rows through the sample presenter, can display default unavailable results or the guarded backend-only result, and adds no Apply button, settings controls, or lifecycle behavior. The focused usage recipes in [`sqlui_persistence_settings_draft_umg_usage.md`](sqlui_persistence_settings_draft_umg_usage.md), [`sqlui_persistence_settings_apply_preview_umg_usage.md`](sqlui_persistence_settings_apply_preview_umg_usage.md), [`sqlui_persistence_settings_apply_contract_umg_usage.md`](sqlui_persistence_settings_apply_contract_umg_usage.md), and [`sqlui_persistence_settings_apply_result_umg_usage.md`](sqlui_persistence_settings_apply_result_umg_usage.md) record how future Blueprint subclasses can bind to those rows without adding apply/save/reset behavior or persistence lifecycle side effects.
...
The dedicated next-phase plan for mutating settings and reset UX lives in [`sqlui_persistence_settings_editing_reset_plan.md`](sqlui_persistence_settings_editing_reset_plan.md). PR #105 recorded that plan only; the first follow-ups implement the draft/validation model, dry-run apply-intent preview, apply/cancel contract, callable apply entrypoint, validation display rows, apply-preview display rows, apply/cancel contract display rows, apply-result display rows, SQLUISamples sample/dev adapters for validation, apply-preview, apply/cancel contract, and apply-result rows, optional C++ UMG widget shell contracts for those rows, and safe UMG usage guides. The #105-#131 apply entrypoint/result UI foundation is complete as a display/reporting checkpoint, and the only mutating extension is the explicit backend-only production apply request. Backend selector UI, SQLite path editor UI, provider auto-init controls, broad settings apply/save execution, reset/delete behavior, and product startup policy remain future work.

## Draft Validation Foundation Checkpoint

The non-mutating persistence settings apply/cancel contract UI foundation is complete as a safe base for future Apply/Cancel implementation work. It includes the #105 editing/reset UX plan, #106 validation-only draft model, #107 validation display rows/summary, #108 optional SQLUISamples draft validation presenter/adapter, #109 optional C++ UMG widget shell, #110 safe UMG usage guide, #111 final draft-validation checkpoint, #112 dry-run apply-intent preview, #113 UI-safe apply-preview display rows/summary, #114 optional SQLUISamples apply-preview adapter, #115 optional apply-preview C++ UMG shell, #116 apply-preview usage guide, #117 final apply-preview checkpoint, #118 SQLUICore non-mutating apply/cancel contract, #119 UI-safe apply/cancel contract display rows/summary, #120 editor validation checkpoint, #121 optional SQLUISamples apply/cancel contract presenter/adapter, #122 optional apply/cancel contract C++ UMG widget shell, #123 apply/cancel contract UMG usage guide, #124 final docs-only foundation checkpoint, and `-UsePersistenceSettingsDraftProbe` smoke coverage.

This checkpoint still does not implement a product settings screen. It adds no settings controls, backend selector UI, SQLite path editor UI, provider auto-init controls, broad settings apply/save/config-write behavior, reset/delete actions, widget blueprint assets, maps, startup wiring, viewport attachment, polling, ticking, auto-refresh, provider/repository initialization, migrations, seed-copy behavior, or default config changes. The draft validation, apply-preview, apply/cancel contract, apply result display, and display paths remain caller-invoked and display/report-only; only the explicit backend-only production apply request can write the selected settings file.

The dry-run apply-intent preview, apply/cancel contract, apply entrypoint, apply-preview display rows, apply/cancel contract display rows, apply-result display rows, optional SQLUISamples apply-preview presenter, optional SQLUISamples apply/cancel contract presenter, optional SQLUISamples apply-result presenter, optional apply-preview widget shell, optional apply/cancel contract widget shell, and optional apply-result widget shell report what a future Apply would do, including whether a draft is valid enough to apply, whether it has changes, whether backend/SQLite/provider-auto-init policy would change, and whether restart/reopen/reinitialize messaging should be shown. Default Apply execution is still unavailable/not implemented. The explicit backend-only production apply request can write only the backend value while keeping provider/repository/database/file side-effect flags false except for the selected settings file directory. The apply-result display rows, sample presenter, and C++ widget shell format those result flags as safe user-readable statements such as "Config written," "Config not written," "Settings changed," "Settings not changed," "Provider not initialized," "Database files not created," and "Database not opened for writing." Cancel/discard is represented as value-preview data only. The safe binding recipes are documented in [`sqlui_persistence_settings_apply_preview_umg_usage.md`](sqlui_persistence_settings_apply_preview_umg_usage.md) for the apply-preview shell, [`sqlui_persistence_settings_apply_contract_umg_usage.md`](sqlui_persistence_settings_apply_contract_umg_usage.md) for the apply/cancel contract shell, and [`sqlui_persistence_settings_apply_result_umg_usage.md`](sqlui_persistence_settings_apply_result_umg_usage.md) for the apply-result shell. They use "would change," "not implemented," "not applied," "config written/not written," and "not saved" semantics and do not create database files, open databases for writing, run migrations, copy seed databases, initialize providers/repositories, or delete files except smoke-owned and probe-created settings-file cleanup in tests.

The #117 apply-preview foundation checkpoint declared the #112-#116 dry-run apply-preview UI chain complete. The apply/cancel contract checkpoint through #124 declares the #118-#124 readiness/display/adapter/widget-shell/usage/final-checkpoint chain complete. These checkpoints still do not add broad mutating Apply/Cancel execution, settings editing controls, backend selector controls, SQLite path editing, provider auto-init controls, reset/delete behavior, widget blueprint assets, maps, startup wiring, viewport attachment, polling, ticking, auto-refresh, migrations, seed-copy behavior, provider/repository initialization, or default policy changes.

The apply-result checkpoint through #131 declared the #125-#131 actual apply gate, entrypoint skeleton, result display rows, sample adapter, C++ UMG shell, usage-guide, and final checkpoint chain complete before any real Apply button behavior or config-write path. That foundation now has one backend-only write extension. It can report unavailable/not implemented, validation-blocked, no-change, preview-only, succeeded, and side-effect flags, but it still does not add settings controls, apply settings to live provider/repository state, create DBs, open databases for writing, run migrations, copy seeds, initialize providers/repositories, delete files, or change startup behavior.

The first write-capable follow-up is intentionally not UI. SQLUICore now has an explicit smoke-owned apply config target scaffold for isolated tests, plus the #134 target policy/resolver skeleton that makes target choice explicit. `-UsePersistenceSettingsDraftProbe` verifies that a temporary `Saved/SQLUI/SmokeTests` ini target can be written and cleaned up, and that the selected production target can be written only through an explicit backend-only request. That production request writes only `Backend=<value>` to `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini`, refuses invalid drafts without mutation, treats no-change requests as no-ops, rejects provider auto-init changes, preserves repo `Config`/`Saved\Config`, and cleans up the probe-created target. No settings controls exist, and future UI must continue treating Apply output as report data until broader settings-editing behavior is separately scoped.

PR #145 adds the matching backend-only readback checkpoint, but it is still not UI. It reads only `Backend` from `RuntimeSettings.ini` when explicitly called, reports absent/valid/invalid states, and does not apply settings, initialize providers/repositories, create databases, run migrations, copy seeds, delete files, or change startup behavior. Future settings UI should treat that readback as display/preview input only until a later startup or provider lifecycle PR explicitly scopes consumption.

The production config target strategy gate keeps broad Apply closed. It explicitly rejects runtime writes to committed defaults such as `DefaultEngine.ini`, widget-owned config writes, surprise user/global editor writes, implicit `Saved/Config` writes, and any target that silently changes startup behavior, provider lifecycle, or SQLite database files. SQLUICore can resolve that documented strategy and describe `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` as the SQLUICore-owned settings file. The first implementation writes only the backend value through an explicit guarded request; it does not expose a general writable `ConfigFilePath`, write SQLite path/provider auto-init/migration/seed/reset/delete settings, initialize providers/repositories, create or open databases, or change startup behavior. Broader production Apply work still needs separate validation and packaged validation when startup or packaged runtime behavior can be affected.

The production config target resolution, guarded enablement, and backend-only write/readback checkpoints do not change the future UI contract. They make the selected target and explicit enablement request visible to SQLUICore policy, and they prove one narrow backend-only write plus one narrow backend-only parse, but they do not add a settings screen, broad production Apply, or startup consumption. Future UI must still not expose backend selector controls, SQLite path editing, provider auto-init toggles, Apply/Save controls, or reset/delete controls as mutating actions until each SQLUICore write/readback path, validation, no-op, config preservation, lifecycle-separation, and smoke coverage requirement is implemented.

The #139 guarded enablement checkpoint and the descriptor checkpoint are policy proofs, not UI readiness signals. They prove an explicit production target enablement request can be represented and that the selected target can be described without inferring unsafe config locations. The backend-only write/readback slices prove the selected `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` target can be touched narrowly, but the product UI must still not expose Apply/Save/config-write controls for provider auto-init, SQLite paths, migrations, seeds, reset/delete, or lifecycle behavior until those paths are separately scoped and validated.

## Backend-Only Apply Write UX Checkpoint

The backend-only write/readback checkpoint is useful for future UI because it proves the SQLUICore apply boundary can perform one narrow, validated production-target write and one narrow backend-only parse without giving widgets ownership of config or persistence lifecycle. A future panel may display backend-only result or readback state, but it should not treat the checkpoint as permission to add mutating controls or startup consumption.

For UI planning, the important constraints are:

- The only selected-target setting written today is `Backend`.
- The only selected-target setting read back today is `Backend`, and readback is caller-invoked only.
...
- Reading `Backend=SQLite` does not apply SQLite to the active provider/repository, initialize lifecycle state, create databases, or read SQLite path/provider auto-init settings.
...
SQLUICore now owns the actual apply entrypoint through `RequestPersistenceSettingsApply`, and `-UsePersistenceSettingsDraftProbe` smoke-tests both the default unavailable/non-mutating path and the explicit backend-only production write path. This is still not real Apply button behavior for a settings screen: it gives future UI a safe SQLUICore call boundary and explicit result data, but the only production setting it can write is the backend value. It does not initialize providers or repositories, create DB files, open SQLite for writing, run migrations, copy seeds, delete files, write SQLite path/provider auto-init, or change startup behavior.
...
- No settings editing or reset/delete controls are added by the optional SQLUISamples status presenter, status panel adapter, status UMG widget shell, draft validation presenter, draft validation UMG widget shell, apply-preview presenter, apply/cancel contract presenter, apply-preview UMG widget shell, or apply/cancel contract UMG widget shell.
...
The draft settings model, apply-intent preview, apply/cancel contract, apply-preview display rows, apply/cancel contract display rows, validation display rows, sample adapters, and C++ widget shells may be used by future controls, but adding them does not add those controls. UI surfaces should keep treating draft validation, apply preview, and apply/cancel contract data as dry-run/pending-state steps until a later PR adds explicit mutating Apply/Cancel behavior.
