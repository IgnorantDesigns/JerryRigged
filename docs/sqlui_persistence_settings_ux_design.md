...
The backend and policy pieces now exist for a safe first UI. Tiny optional SQLUISamples sample/dev presenter, panel-adapter, and C++ UMG widget shell objects already provide Blueprint-callable, explicitly refreshed read-only display-row hooks for validation and Blueprint/sample consumption, and SQLUICore now has dry-run apply-intent preview plus apply/cancel contract and apply-result display rows for drafts. SQLUICore also has the #134 apply config target policy/resolver skeleton plus a documented production target strategy resolver, which keep default/runtime Apply unavailable, keep the explicit smoke-owned target as the only write-capable target, and leave a future project/user target represented but non-writable until it is separately scoped. The selected production target descriptor identifies `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` as SQLUICore/plugin-managed, non-committed, not `DefaultEngine.ini`, not generated `Saved/Config`, and not user/global editor settings, but it is still write-disabled and does not create the file or directory. The guarded production target enablement request records an explicit request but still reports blocked/non-writable because the selected production target is descriptor-only and not implemented. SQLUISamples also has sample/dev adapters and optional C++ UMG widget shells for validation, apply-preview, apply/cancel contract, and apply-result rows, but the full product settings UI is still future work. The actual apply/save/config-write implementation is gated in [`sqlui_persistence_settings_editing_reset_plan.md`](sqlui_persistence_settings_editing_reset_plan.md) and must be SQLUICore-first before real controls are introduced. This design keeps SQLite explicit, keeps `InMemory` as the safe default, and routes database status/reset behavior through SQLUICore helpers instead of widget-owned storage logic.
...
The first settings-editing foundation slice now exists as `FSQLUIPersistenceSettingsDraft`, `FSQLUIPersistenceSettingsValidationResult`, `FSQLUIPersistenceSettingsApplyPreviewResult`, `FSQLUIPersistenceSettingsApplyContractResult`, `FSQLUIPersistenceSettingsCancelPreviewResult`, `FSQLUIPersistenceSettingsApplyRequest`, `FSQLUIPersistenceSettingsApplyResult`, and `USQLUIPersistenceSettingsDraftLibrary`. It is still non-mutating: callers can represent current and pending runtime settings, validate backend/path/provider-auto-init choices, reset the in-memory draft value back to current values, build a dry-run apply-intent preview, query non-executing future-apply readiness, describe cancel/discard as a pure value preview, and call an actual apply entrypoint skeleton that returns unavailable/not implemented without mutation. Draft validation, apply-preview generation, apply/cancel contract generation, apply-request skeleton execution, apply-result display generation, and their display-row formatters do not apply settings, write config, create database files or directories, open databases for writing, run migrations, copy seed databases, initialize providers/repositories, delete files, add UI controls, or change startup behavior. `USQLUIPersistenceSettingsDraftDisplayLibrary`, `USQLUIPersistenceSettingsApplyPreviewDisplayLibrary`, `USQLUIPersistenceSettingsApplyContractDisplayLibrary`, and `USQLUIPersistenceSettingsApplyResultDisplayLibrary` add UI-safe summary/row formatters for validation, apply-preview, apply/cancel contract, and apply result output so a future sample or settings panel can show pending backend/path/provider-auto-init status, future apply readiness, explicit unavailable/not-implemented apply results, and cancel/discard value preview without owning apply, save, or lifecycle behavior.

`USQLUISamplePersistenceSettingsDraftPresenter`, `USQLUISamplePersistenceSettingsApplyPreviewPresenter`, `USQLUISamplePersistenceSettingsApplyContractPresenter`, `USQLUISamplePersistenceSettingsApplyResultPresenter`, `USQLUISamplePersistenceSettingsDraftPanelWidget`, `USQLUISamplePersistenceSettingsApplyPreviewPanelWidget`, `USQLUISamplePersistenceSettingsApplyContractPanelWidget`, and `USQLUISamplePersistenceSettingsApplyResultPanelWidget` are optional SQLUISamples sample/dev surfaces for those draft validation, apply-preview, apply/cancel contract, and apply-result rows. The widget shells are C++ `UUserWidget` subclasses only: they create no visual layout, add no widget blueprint asset, are not added to the viewport, are not wired into maps/startup/config, and do not refresh from lifecycle hooks. Their caller-invoked refresh/build functions delegate to presenters, cache rows/result/summary in memory, and remain validation/preview/display-only. The apply-result shell consumes the SQLUICore apply entrypoint skeleton/display rows through the sample presenter, keeps the latest unavailable/non-mutating result in memory, and adds no Apply button, settings controls, or lifecycle behavior. The focused usage recipes in [`sqlui_persistence_settings_draft_umg_usage.md`](sqlui_persistence_settings_draft_umg_usage.md), [`sqlui_persistence_settings_apply_preview_umg_usage.md`](sqlui_persistence_settings_apply_preview_umg_usage.md), [`sqlui_persistence_settings_apply_contract_umg_usage.md`](sqlui_persistence_settings_apply_contract_umg_usage.md), and [`sqlui_persistence_settings_apply_result_umg_usage.md`](sqlui_persistence_settings_apply_result_umg_usage.md) record how future Blueprint subclasses can bind to those rows without adding apply/save/reset behavior or persistence lifecycle side effects.
...
The dedicated next-phase plan for mutating settings and reset UX lives in [`sqlui_persistence_settings_editing_reset_plan.md`](sqlui_persistence_settings_editing_reset_plan.md). PR #105 recorded that plan only; the first follow-ups implement only the non-mutating draft/validation model, dry-run apply-intent preview, non-mutating apply/cancel contract, callable apply entrypoint skeleton that still refuses execution, validation display rows, apply-preview display rows, apply/cancel contract display rows, apply-result display rows, SQLUISamples sample/dev adapters for validation, apply-preview, apply/cancel contract, and apply-result rows, optional C++ UMG widget shell contracts for those rows, and safe UMG usage guides. The #105-#131 apply entrypoint/result UI foundation is now a complete non-mutating checkpoint. Backend selector UI, SQLite path editor UI, provider auto-init controls, actual settings apply/save execution, reset/delete behavior, and product startup policy remain future work.
...
This checkpoint still does not implement a product settings screen. It adds no settings controls, backend selector UI, SQLite path editor UI, provider auto-init controls, actual settings apply/save/config-write behavior, reset/delete actions, widget blueprint assets, maps, startup wiring, viewport attachment, polling, ticking, auto-refresh, provider/repository initialization, migrations, seed-copy behavior, or default config changes. The draft validation, apply-preview, apply/cancel contract, apply result display, and display paths remain caller-invoked, display/report-only, and non-mutating.

The dry-run apply-intent preview, apply/cancel contract, actual apply entrypoint skeleton, apply-preview display rows, apply/cancel contract display rows, apply-result display rows, optional SQLUISamples apply-preview presenter, optional SQLUISamples apply/cancel contract presenter, optional SQLUISamples apply-result presenter, optional apply-preview widget shell, optional apply/cancel contract widget shell, and optional apply-result widget shell report what a future Apply would do, including whether a draft is valid enough to apply, whether it has changes, whether backend/SQLite/provider-auto-init policy would change, whether restart/reopen/reinitialize messaging should be shown, and that actual Apply execution is still unavailable/not implemented. The apply entrypoint skeleton returns validation-blocked, no-change, or preview-only/unavailable results and keeps every config/settings/provider/repository/database/file side-effect flag false. The apply-result display rows, sample presenter, and C++ widget shell format those result flags as safe user-readable statements such as "Config not written," "Settings not changed," "Provider not initialized," "Database files not created," and "Database not opened for writing." Cancel/discard is represented as value-preview data only. The safe binding recipes are documented in [`sqlui_persistence_settings_apply_preview_umg_usage.md`](sqlui_persistence_settings_apply_preview_umg_usage.md) for the apply-preview shell, [`sqlui_persistence_settings_apply_contract_umg_usage.md`](sqlui_persistence_settings_apply_contract_umg_usage.md) for the apply/cancel contract shell, and [`sqlui_persistence_settings_apply_result_umg_usage.md`](sqlui_persistence_settings_apply_result_umg_usage.md) for the apply-result shell. They use "would change," "not implemented," "not applied," "config not written," and "not saved" semantics and do not write config, create directories or database files, open databases for writing, run migrations, copy seed databases, initialize providers/repositories, or delete files except smoke-owned cleanup in tests.
...
The apply-result checkpoint through #131 declares the #125-#131 actual apply gate, entrypoint skeleton, result display rows, sample adapter, C++ UMG shell, usage-guide, and final checkpoint chain complete before any real Apply button behavior or config-write path. It remains display/refusal-only: it can report unavailable/not implemented, validation-blocked, no-change, preview-only, and side-effect flags, but it does not write config, apply settings, create directories or DBs, open databases for writing, run migrations, copy seeds, initialize providers/repositories, delete files, or change startup behavior.

The first write-capable follow-up is intentionally not UI. SQLUICore now has an explicit smoke-owned apply config target scaffold for isolated tests, plus the #134 non-mutating target policy/resolver skeleton that makes target choice explicit. `-UsePersistenceSettingsDraftProbe` verifies that only a temporary `Saved/SQLUI/SmokeTests` ini target can be written and cleaned up; default/runtime targets resolve unavailable, and the future project/user target remains unavailable/not implemented. That checkpoint proves target isolation, draft-validation-before-write, invalid-draft refusal without mutation, repo `Config`/`Saved\Config` preservation, and cleanup of the smoke-owned artifact. The real user/runtime Apply path is still unavailable/not implemented, no settings controls exist, and future UI must continue treating Apply output as report data until a separately scoped production apply/config-write path exists.

The production config target strategy gate keeps that path closed for now. It explicitly rejects runtime writes to committed defaults such as `DefaultEngine.ini`, widget-owned config writes, surprise user/global editor writes, implicit `Saved/Config` writes, and any target that silently changes startup behavior, provider lifecycle, or SQLite database files. SQLUICore can now resolve that documented strategy as a future project/user target kind with no writable path and production Apply still disabled, and it can describe `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` as the future SQLUICore-owned settings file. The code does not write that file, create its directory, or expose it as a writable `ConfigFilePath` yet. The safest next strategy remains to keep production Apply unavailable until a SQLUICore policy PR implements that target and proves draft validation, no-op behavior, config preservation, smoke-owned versus real-target separation, no database side effects, no provider/repository lifecycle work, and packaged validation when startup or packaged runtime behavior can be affected.

The production config target resolution and guarded enablement checkpoints do not change the future UI contract. They only make the documented future real target and explicit enablement request visible to SQLUICore policy while keeping production Apply unavailable/non-writable. Future UI must still not expose backend selector controls, SQLite path editing, provider auto-init toggles, Apply/Save controls, or reset/delete controls as mutating actions until the SQLUICore write target, validation, no-op, config preservation, lifecycle-separation, and smoke coverage requirements are implemented.

The #139 guarded enablement checkpoint and the descriptor checkpoint are therefore policy proofs, not UI readiness signals. They prove an explicit production target enablement request can be recorded and blocked while the concrete target remains unimplemented, and that the selected target can be described without creating files or enabling writes. The product UI must continue to present Apply/Save/config-write behavior as unavailable until a later SQLUICore PR implements the selected `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` target, keeps smoke-owned and production paths separate, and validates invalid/no-change/write paths without DB, provider, repository, migration, seed-copy, startup, or default-policy side effects.
...
SQLUICore now owns the first actual apply entrypoint skeleton through `RequestPersistenceSettingsApply`, and `-UsePersistenceSettingsDraftProbe` smoke-tests that it remains unavailable/not implemented and non-mutating. This is still not real Apply button behavior: it gives future UI a safe SQLUICore call boundary and explicit refusal/result data, but it does not save settings, write config, initialize providers or repositories, create directories or DB files, open SQLite for writing, run migrations, copy seeds, delete files, or change startup behavior.
