...
The backend and policy pieces now exist for a safe first UI. Tiny optional SQLUISamples sample/dev presenter, panel-adapter, and C++ UMG widget shell objects already provide Blueprint-callable, explicitly refreshed read-only display-row hooks for validation and Blueprint/sample consumption, and SQLUICore now has dry-run apply-intent preview plus apply/cancel contract display rows for drafts, but the full product settings UI is still future work. This design keeps SQLite explicit, keeps `InMemory` as the safe default, and routes database status/reset behavior through SQLUICore helpers instead of widget-owned storage logic.
...
`USQLUISamplePersistenceSettingsDraftPresenter`, `USQLUISamplePersistenceSettingsDraftPanelWidget`, `USQLUISamplePersistenceSettingsApplyPreviewPresenter`, and `USQLUISamplePersistenceSettingsApplyPreviewPanelWidget` are optional SQLUISamples sample/dev surfaces for those draft validation and apply-preview rows. The widget shells are C++ `UUserWidget` subclasses only: they create no visual layout, add no widget blueprint asset, are not added to the viewport, are not wired into maps/startup/config, and do not refresh from lifecycle hooks. Their caller-invoked refresh/build functions delegate to presenters, cache rows/result/summary in memory, and remain validation/preview/display-only. The focused usage recipes in [`sqlui_persistence_settings_draft_umg_usage.md`](sqlui_persistence_settings_draft_umg_usage.md) and [`sqlui_persistence_settings_apply_preview_umg_usage.md`](sqlui_persistence_settings_apply_preview_umg_usage.md) record how future Blueprint subclasses can bind to those rows without adding apply/save/reset behavior or persistence lifecycle side effects.
...
- No settings editing or reset/delete controls are added by the optional SQLUISamples status presenter, status panel adapter, status UMG widget shell, draft validation presenter, draft validation UMG widget shell, apply-preview presenter, or apply-preview UMG widget shell.
