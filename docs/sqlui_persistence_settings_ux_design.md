...
The #139 guarded enablement checkpoint is therefore a policy proof, not a UI readiness signal. It proves an explicit production target enablement request can be recorded and blocked while the concrete target remains unimplemented. The product UI must continue to present Apply/Save/config-write behavior as unavailable until a later SQLUICore PR implements the selected `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` target, keeps smoke-owned and production paths separate, and validates invalid/no-change/write paths without DB, provider, repository, migration, seed-copy, startup, or default-policy side effects.
