...
The first write-capable follow-up is intentionally not UI. SQLUICore now has an explicit smoke-owned apply config target scaffold for isolated tests, and `-UsePersistenceSettingsDraftProbe` verifies that only a temporary `Saved/SQLUI/SmokeTests` ini target can be written and cleaned up. That checkpoint proves target isolation, draft-validation-before-write, invalid-draft refusal without mutation, repo `Config`/`Saved\Config` preservation, and cleanup of the smoke-owned artifact. The real user/runtime Apply path is still unavailable/not implemented, no settings controls exist, and future UI must continue treating Apply output as report data until a separately scoped production apply/config-write path exists.
