...
The dry-run apply-intent preview, apply-preview display rows, optional SQLUISamples apply-preview presenter, and optional apply-preview widget shell report what a future Apply would do, including whether a draft is valid enough to apply, whether it has changes, whether backend/SQLite/provider-auto-init policy would change, and whether restart/reopen/reinitialize messaging should be shown. The safe binding recipe for that apply-preview shell is documented in [`sqlui_persistence_settings_apply_preview_umg_usage.md`](sqlui_persistence_settings_apply_preview_umg_usage.md). They use "would change" and "not applied" semantics and do not write config, create directories or database files, open databases for writing, run migrations, copy seed databases, initialize providers/repositories, or delete files except smoke-owned cleanup in tests.

The follow-up #117 apply-preview foundation checkpoint declares the #112-#116 dry-run apply-preview UI chain complete. That checkpoint is docs-only and does not add actual Apply/Cancel helpers, config writes, settings editing controls, backend selector controls, SQLite path editing, provider auto-init controls, reset/delete behavior, widget blueprint assets, maps, startup wiring, viewport attachment, polling, ticking, auto-refresh, migrations, seed-copy behavior, provider/repository initialization, or default policy changes.
...
The draft settings model, apply-intent preview, apply-preview display rows, validation display rows, sample adapters, and C++ widget shells may be used by future controls, but adding them does not add those controls. UI surfaces should keep treating draft validation and apply preview as dry-run/pending-state steps until a later PR adds explicit Apply/Cancel behavior.
