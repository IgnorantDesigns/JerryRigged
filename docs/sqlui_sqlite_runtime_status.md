...
- `-UsePersistenceStatusSampleSurfaceProbe`: verifies the optional SQLUISamples presenter exposes Blueprint-callable refresh hooks, consumes display rows, exposes formatted lines, performs explicit and repeated refresh deterministically without creating files, handles missing SQLite DB state gracefully, preserves a smoke-owned sidecar during refresh, and cleans up.
