...
PR #134 adds a non-mutating apply config target policy/resolver skeleton. It resolves the production/default target as unavailable/not implemented, resolves an explicit safe smoke-owned target as the only write-capable target, and represents a future project/user config target as unavailable until a later PR explicitly scopes and validates it. The documented production target strategy is now also represented by `ResolveDocumentedProductionTargetStrategy()`, which resolves to the same future project/user target kind with no writable path, `bCanWrite=false`, and production Apply disabled. The selected future target design is a SQLUICore-owned `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` settings file for validated persistence settings intent only, but no code writes it yet. `ResolveDocumentedProductionTargetStrategyWithEnablement()` can record an explicit production target enablement request, but the request remains blocked/non-writable/pathless while that selected target is unimplemented. `RequestPersistenceSettingsApply` remains the existing unavailable/non-mutating apply entrypoint and still writes no config or changes settings. The resolver does not infer `DefaultEngine.ini`, `Saved/Config`, user/global editor settings, or any other real writable production target.
...
- The selected SQLUICore-owned settings target is `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini`, but no production helper/format writes it yet.
