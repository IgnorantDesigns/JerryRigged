...
Actual mutating apply/save/config-write behavior remains future work. The runtime currently has non-mutating draft validation, dry-run apply preview, apply/cancel contract, a non-mutating apply request skeleton, display rows, SQLUISamples adapters, and C++ widget shells.
