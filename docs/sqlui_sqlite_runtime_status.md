...
`USQLUISamplePersistenceStatusPresenter` is the first optional SQLUISamples sample/dev surface over those display rows. It refreshes rows from SQLUICore and stores stable formatted strings for simple sample UI, Blueprint, or commandlet presentation. It is not a full settings screen, is not wired into default startup, and does not add settings editing, reset/delete actions, provider/repository initialization, database creation, migrations, seed copy, or file deletion.
...
- `-UsePersistenceStatusSampleSurfaceProbe`: verifies the optional SQLUISamples presenter consumes display rows, exposes formatted lines, handles missing SQLite DB state gracefully without creating files, preserves a smoke-owned sidecar during refresh, and cleans up.
