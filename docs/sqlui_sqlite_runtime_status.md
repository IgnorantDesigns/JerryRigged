...
`USQLUISamplePersistenceStatusPresenter` is the first optional SQLUISamples sample/dev surface over those display rows. Its explicit caller-invoked refresh re-queries rows from SQLUICore, stores stable formatted strings for simple sample UI, Blueprint, or commandlet presentation, and returns a small refresh result for sample/dev consumers. It is not a full settings screen, is not wired into default startup, maps, config, timers, tick, or polling, and does not add settings editing, reset/delete actions, provider/repository initialization, database creation, migrations, seed copy, or file deletion.
...
- `-UsePersistenceStatusSampleSurfaceProbe`: verifies the optional SQLUISamples presenter consumes display rows, exposes formatted lines, performs explicit and repeated refresh deterministically without creating files, handles missing SQLite DB state gracefully, preserves a smoke-owned sidecar during refresh, and cleans up.
