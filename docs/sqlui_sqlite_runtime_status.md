...
PR #134 adds a non-mutating apply config target policy/resolver skeleton. It resolves the production/default target as unavailable/not implemented, resolves an explicit safe smoke-owned target as the only write-capable target, and represents a future project/user config target as unavailable until a later PR explicitly scopes and validates it. The documented production target strategy is now also represented by `ResolveDocumentedProductionTargetStrategy()`, which resolves to the same future project/user target kind with no writable path, `bCanWrite=false`, and production Apply disabled. SQLUICore now also exposes a descriptor for the selected future target design, a SQLUICore-owned `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` settings file for validated persistence settings intent only. The descriptor reports that it is not committed config, not `DefaultEngine.ini`, not generated `Saved/Config`, not user/global editor settings, not smoke-owned, not implemented for writes, and not write-enabled. `ResolveDocumentedProductionTargetStrategyWithEnablement()` can record an explicit production target enablement request, but the request remains blocked/non-writable while that selected target is unimplemented. `RequestPersistenceSettingsApply` remains the existing unavailable/non-mutating apply entrypoint and still writes no config or changes settings. The resolver does not infer `DefaultEngine.ini`, `Saved/Config`, user/global editor settings, or any other real writable production target, and it does not create the selected target file or directory.
...
- The selected SQLUICore-owned settings target is described as `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini`, but no production helper/format writes it yet and the file/directory are not created.
- Explicit smoke-owned `Saved/SQLUI/SmokeTests` targets remain the only write-capable path today.
- Explicit production target enablement requests are recorded as blocked/non-writable while the selected target remains unimplemented.
...
The checkpoint includes the #105 editing/reset UX plan, #106 validation-only draft model, #107 validation display rows, #108 draft validation adapter, #109 draft validation C++ UMG shell, #110 draft validation usage guide, #111 draft validation checkpoint, #112 dry-run apply preview, #113 apply-preview display rows, #114 apply-preview adapter, #115 apply-preview C++ UMG shell, #116 apply-preview usage guide, #117 apply-preview checkpoint, #118 non-mutating apply/cancel contract, #119 apply/cancel contract display rows, #120 editor launch validation checkpoint, #121 apply/cancel contract adapter, #122 apply/cancel contract C++ UMG shell, #123 apply/cancel contract usage guide, #124 final docs-only foundation checkpoint, #126 apply entrypoint skeleton, #127 apply-result display rows, the SQLUISamples apply-result sample adapter, the optional apply-result C++ UMG shell, and `-UsePersistenceSettingsDraftProbe` smoke coverage for the validation/apply-preview/apply-contract/apply-result model, display rows, sample adapters, and widget-shell contracts.
...
- `-UsePersistenceSettingsDraftProbe`: verifies the SQLUICore draft/validation model, dry-run apply preview, non-mutating apply/cancel contract, apply entrypoint skeleton, validation display rows, apply-preview display rows, apply/cancel contract display rows, apply-result display rows, the smoke-owned apply config target scaffold, the non-mutating apply config target policy/resolver skeleton, SQLUISamples validation/apply-preview/apply-contract/apply-result sample adapters, and optional validation/apply-preview/apply-contract/apply-result C++ UMG widget shell reflection contracts for default/current `InMemory`, unknown backend rejection/error display/preview-display/blocked-contract/contract-display/apply-result-display/apply-result-adapter/apply-result-widget error, SQLite draft/path display/preview-display/contract/contract-display/apply-result-display/apply-result-adapter/apply-result-widget without DB creation, empty SQLite path rejection/error display/preview-display/blocked-contract/contract-display error, provider auto-init pending validation/display/preview-display/contract/contract-display/apply-result-display/apply-result-adapter/apply-result-widget without policy mutation, deterministic validation/display/preview/preview-display/contract-display/apply-result-display/apply-result-adapter/apply-result-widget/contract/adapter output, default/runtime target unavailable, explicit smoke-owned target write capability, future real target unavailable, documented production strategy non-writable/no-writable-path resolution, selected production descriptor resolution for `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini`, no descriptor target file/directory creation, guarded production target enablement request/no-request blocked behavior, smoke-owned sidecar preservation, and cleanup.
