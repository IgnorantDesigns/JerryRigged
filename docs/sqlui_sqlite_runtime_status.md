...
- `-UsePersistenceSettingsDraftProbe`: verifies the SQLUICore draft/validation model and validation display rows for default/current `InMemory`, unknown backend rejection/error display, SQLite draft/path display without DB creation, empty SQLite path rejection/error display, provider auto-init pending validation/display without policy mutation, deterministic validation/display, smoke-owned sidecar preservation, and cleanup.
