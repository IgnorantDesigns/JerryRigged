...
PR #134 adds a non-mutating apply config target policy/resolver skeleton. It resolves the production/default target as unavailable/not implemented, resolves an explicit safe smoke-owned target as the only write-capable target, and represents a future project/user config target as unavailable until a later PR explicitly scopes and validates it. The documented production target strategy is now also represented by `ResolveDocumentedProductionTargetStrategy()`, which resolves to the same future project/user target kind with no writable path, `bCanWrite=false`, and production Apply disabled. `ResolveDocumentedProductionTargetStrategyWithEnablement()` can record an explicit production target enablement request, but the request remains blocked/non-writable/pathless while no concrete safe target is selected. `RequestPersistenceSettingsApply` remains the existing unavailable/non-mutating apply entrypoint and still writes no config or changes settings. The resolver does not infer `DefaultEngine.ini`, `Saved/Config`, user/global editor settings, or any other real writable production target.
...
- Explicit production target enablement requests are recorded as blocked/non-writable while no safe concrete production target exists.
