...
`USQLUIPersistenceSettingsDraftLibrary` is the first SQLUICore settings draft surface for future persistence UI. It can build current/default `FSQLUIPersistenceSettingsDraft` values, represent pending backend/path/provider-auto-init changes, reset a draft value back to current values, validate pending values through `FSQLUIPersistenceSettingsValidationResult`, build `FSQLUIPersistenceSettingsApplyPreviewResult` dry-run apply-intent previews that report what a future Apply would do, report non-executing future-apply readiness through `FSQLUIPersistenceSettingsApplyContractResult`, describe cancel/discard as pure value state through `FSQLUIPersistenceSettingsCancelPreviewResult`, and accept `FSQLUIPersistenceSettingsApplyRequest` through a callable apply entrypoint. Default `RequestPersistenceSettingsApply` calls reuse the same contract data and return validation-blocked, no-change, or preview-only/unavailable `FSQLUIPersistenceSettingsApplyResult` values while keeping `bSucceeded=false`, `bActualApplyImplemented=false`, and every side-effect flag false. Explicit backend-only production requests are the narrow exception and can write only `Backend=<value>` to `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini`. SQLUICore also has an explicit backend-only readback helper for that selected target: it reads only `Backend`, treats an absent file as no runtime override, reports missing/unknown backend values as errors, and does not apply settings, initialize providers/repositories, create databases, run migrations, copy seeds, delete files, or run at startup. `USQLUIPersistenceSettingsDraftDisplayLibrary`, `USQLUIPersistenceSettingsApplyPreviewDisplayLibrary`, `USQLUIPersistenceSettingsApplyContractDisplayLibrary`, and `USQLUIPersistenceSettingsApplyResultDisplayLibrary` format validation-only, apply-preview, apply/cancel contract, and apply-result output into UI-safe summary/row data for future sample or settings panels. `USQLUISamplePersistenceSettingsDraftPresenter`, `USQLUISamplePersistenceSettingsApplyPreviewPresenter`, `USQLUISamplePersistenceSettingsApplyContractPresenter`, and `USQLUISamplePersistenceSettingsApplyResultPresenter` are optional SQLUISamples sample/dev adapters that consume the validation/apply-preview/apply-contract/apply-result SQLUICore rows and keep the latest display result in memory for future sample UI work. `USQLUISamplePersistenceSettingsDraftPanelWidget`, `USQLUISamplePersistenceSettingsApplyPreviewPanelWidget`, `USQLUISamplePersistenceSettingsApplyContractPanelWidget`, and `USQLUISamplePersistenceSettingsApplyResultPanelWidget` are optional SQLUISamples C++ `UUserWidget` shells over the validation/apply-preview/apply-contract/apply-result presenters for future Blueprint binding; they create no visual layout, add no widget blueprint asset, do not attach to the viewport, and do not refresh from lifecycle hooks. The apply-result shell adds no Apply button, settings controls, or lifecycle behavior; it only caches presenter rows/result/summary for caller-invoked display. Validation, apply-preview generation, apply/cancel contract generation, default apply requests, backend-only readback, display formatting, sample adapter refresh, and widget-shell build/refresh do not create database files, open databases for writing, run migrations, copy seed databases, initialize providers/repositories, delete files, add settings controls, or change startup behavior. Cancel/discard remains value-preview only. The focused usage guides are [`sqlui_persistence_settings_draft_umg_usage.md`](sqlui_persistence_settings_draft_umg_usage.md) for draft validation, [`sqlui_persistence_settings_apply_preview_umg_usage.md`](sqlui_persistence_settings_apply_preview_umg_usage.md) for apply-preview display, [`sqlui_persistence_settings_apply_contract_umg_usage.md`](sqlui_persistence_settings_apply_contract_umg_usage.md) for apply/cancel contract display, and [`sqlui_persistence_settings_apply_result_umg_usage.md`](sqlui_persistence_settings_apply_result_umg_usage.md) for apply-result display.

## Persistence Settings Apply Result UI Foundation

The apply entrypoint/result UI foundation is now documented as complete through the #105-#131 chain. That chain covers the settings editing/reset plan, validation-only draft model, validation display rows, SQLUISamples draft adapter, draft C++ UMG shell, draft usage guide, draft checkpoint, dry-run apply preview, apply-preview display rows, apply-preview adapter, apply-preview C++ UMG shell, apply-preview usage guide, apply-preview checkpoint, non-mutating apply/cancel contract, contract display rows, editor validation checkpoint, contract adapter, contract C++ UMG shell, contract usage guide, contract checkpoint, actual apply implementation gate, `RequestPersistenceSettingsApply`, apply-result display rows, apply-result adapter, apply-result C++ UMG shell, apply-result usage guide, final docs-only apply-result checkpoint, and `-UsePersistenceSettingsDraftProbe` smoke coverage.

This foundation is still not settings editing and does not add backend selector controls, SQLite path editor controls, provider auto-init controls, reset/delete UX, widget Blueprint assets, maps, startup wiring, viewport attachment, polling, ticking, timers, auto-refresh, broad settings mutation, provider/repository initialization, DB creation, write-open SQLite access, migrations, seed copy, or file deletion outside smoke-owned cleanup. The only production config mutation is the guarded backend-only write to `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini`; it serializes only `Backend=<value>`. `InMemory` remains the default backend, SQLite remains opt-in, provider auto-init remains off by default, and SQLUICore remains free of UMG, Slate, SlateCore, editor-only, and widget dependencies.

Future broader config-write/apply behavior must stay SQLUICore-first, validate drafts before writing, refuse invalid drafts without mutation, keep widgets away from SQL/schema/migrations/seed-copy/sidecar/deletion/provider lifecycle details, and use explicit pending/apply/cancel semantics. Any reset/delete UX must route through SQLUICore database management policy surfaces. Any new config-write path needs focused smoke coverage plus config diff/snapshot checks, and any startup/default-map/config/provider lifecycle or packaged behavior change needs packaged validation.

## Backend-Only Apply Write Checkpoint

The current runtime status includes one narrow production settings write: an explicit guarded backend-only apply request can write `Backend=<value>` to `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini`. That file is generated settings intent under SQLUICore ownership. It is not committed config, not generated `Saved/Config`, not a SQLite database, not a seed, and not a widget artifact.

The checkpoint does not change runtime startup behavior. Default startup, read-only status snapshots, status display rows, SQLUISamples status presenters/widgets, target descriptor resolution, and guarded enablement resolution do not create `RuntimeSettings.ini` or the parent directory. Writing SQLite as the backend records only the backend value; it does not create database files, create database directories, initialize providers/repositories, open SQLite for writing, run migrations, copy seeds, delete files, or apply live lifecycle changes.

The smoke requirement for this checkpoint is `-UsePersistenceSettingsDraftProbe`, plus cleanup checks. That probe validates valid backend-only write, invalid refusal, provider auto-init rejection for the backend-only scope, no-change/no-op behavior, config snapshot preservation, no DB/lifecycle/migration/seed side effects, and cleanup of the probe-created `RuntimeSettings.ini` and empty parent directory.
...
Default `RequestPersistenceSettingsApply` requests are still unavailable/not implemented and still report `bDidWriteConfig=false` and `bDidChangeSettings=false`. The first mutating path is opt-in and backend-only: when an apply request explicitly enables the backend-only production path, SQLUICore can write only `Backend=<value>` to `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini`. The smoke-owned target helper still does not write committed `Config` defaults or generated `Saved/Config`, and the backend-only production path does not enable SQLite or provider auto-init by default, initialize providers/repositories, create or open SQLite databases, run migrations, copy seeds, or add reset/delete behavior. `-UsePersistenceSettingsDraftProbe` verifies the smoke target write, backend-only production write, no-request/no-change/invalid-draft behavior, unsafe-path refusal, repo config preservation, lifecycle no-op behavior, DB no-op behavior, and smoke/production artifact cleanup.

PR #134 adds an apply config target policy/resolver skeleton. It resolves broad production/default target selection as unavailable/not implemented, resolves an explicit safe smoke-owned target as write-capable for smoke coverage, and represents the documented project/runtime target without exposing an unsafe general writable path. SQLUICore also exposes a descriptor for the selected target design, a SQLUICore-owned `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` settings file for validated persistence settings intent only. The descriptor reports that it is not committed config, not `DefaultEngine.ini`, not generated `Saved/Config`, not user/global editor settings, and not smoke-owned. `ResolveDocumentedProductionTargetStrategyWithEnablement()` can record an explicit production target enablement request, but the resolver alone does not create directories, write files, or make the general production target writable. The only implemented production write is the explicit backend-only apply request, which writes just the backend value and leaves SQLite path/provider auto-init/migration/seed/reset/delete settings out of scope.
...
The selected project/runtime target is implemented only for backend choice. Any additional setting must be implemented as a separate explicit policy decision with validation before write, no-op behavior for unchanged drafts, clear failure messages for invalid drafts, config diff/snapshot checks, separation between smoke-owned and real targets, and no hidden database/lifecycle work in the config-write path. Packaged validation is required if a future implementation changes startup behavior, default maps, config wiring, provider lifecycle, viewport flow, packaged runtime behavior, or committed/default runtime config.

## Production Config Target Strategy

The production config target strategy is now documented and codified as a policy-resolution step before real Apply writes. The current decision is to keep broad production/default Apply unavailable while allowing one explicit backend-only write to the selected `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` target.

The production config target resolution checkpoint records that #137 added a code-level resolution for the documented strategy without changing default behavior. SQLUICore can identify the documented project/runtime target through `ResolveDocumentedProductionTargetStrategy()`, and the descriptor checkpoint names the selected relative path, but the resolver result remains unavailable and non-writable for broad/default Apply. The guarded enablement resolver can also record an explicit request to enable that target, but that resolver alone does not accept the request or write files. The backend-only apply request is the first accepted production write and remains limited to one `Backend` value.

That resolution layer proves the target and explicit enablement request can be represented safely without inferring unsafe config locations. The resolver itself does not write real user/runtime config, apply settings, create or open databases for writing, run migrations, copy seeds, initialize providers/repositories, delete files, add UI controls, change committed config, change startup behavior, make SQLite default, or enable provider auto-init by default. Future write-enable work must keep this policy gate in the write path, validate drafts, preserve no-op behavior, separate smoke-owned and real targets, and include config diff/snapshot checks.

The #139 guarded enablement checkpoint remains policy-only: the request is explicit and smoke-tested, but resolver/descriptor/enablement resolution alone still does not create directories, write files, or make broad production Apply writable. The repository runtime remains `InMemory` by default, SQLite remains opt-in, and provider auto-init remains off by default. The follow-up backend-only production apply slice is the first selected-target writer: it can write just `Backend=<value>` to the SQLUICore-owned `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` storage surface, proves that target is not committed config, `DefaultEngine.ini`, implicit `Saved/Config`, user/global editor settings, or widget-owned storage, and proves config writes do not create DBs or initialize providers/repositories as a side effect.
...
- The selected SQLUICore-owned settings target is described as `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini`; descriptor/policy resolution alone does not create it.
- Explicit smoke-owned `Saved/SQLUI/SmokeTests` targets remain the only full-draft write-capable path.
- Explicit backend-only production apply requests can write only the backend value to `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini`.
- Explicit production target enablement resolver requests are still policy records only; the resolver does not create directories or write files by itself.

Future selected-target write PRs must update SQLUICore policy explicitly, validate drafts before writing, refuse invalid drafts without mutation, preserve no-change/no-op behavior, prove committed config remains unchanged, prove smoke-owned and selected targets remain separate, prove no DB files are created by config Apply, prove no provider/repository lifecycle runs, and keep migrations, seed copy, reset/delete, and database write-open out of config writes. SQLite path writing needs a separate path-safety PR.
...
The first SQLUICore actual apply entrypoint now exists as `RequestPersistenceSettingsApply`. Default requests still report unavailable/not implemented. Explicit backend-only production requests can write only the backend value to the selected SQLUICore-owned settings file. They do not write SQLite path, provider auto-init, migration, seed, reset, or delete settings; they do not initialize providers/repositories, create DB files, open SQLite for writing, run migrations, copy seed databases, delete files, or change startup behavior.

Actual mutating apply/save/config-write behavior remains future work. The runtime currently has non-mutating draft validation, dry-run apply preview, apply/cancel contract, a non-mutating apply request skeleton, apply-result display rows, SQLUISamples adapters including the apply-result presenter, and C++ widget shells including the apply-result shell.

Future mutating apply paths must continue to be implemented through SQLUICore helper/policy surfaces, not through widgets or SQLUISamples. The current backend-only path validates the pending `FSQLUIPersistenceSettingsDraft`, refuses invalid drafts without mutation, writes only an explicitly scoped backend setting, and returns clear user-readable result messages. If no safe write target or scope exists for a future setting, the implementation should keep reporting not-implemented or dry-run-only rather than writing broad or committed config.
...
- `-UsePersistenceSettingsDraftProbe` smoke coverage for the draft model, apply preview, apply/cancel contract, default apply refusal, backend-only production apply write, validation display rows, apply-preview display rows, apply/cancel contract display rows, apply-result display rows, validation/apply-preview/apply-contract/apply-result sample adapters, and validation/apply-preview/apply-contract/apply-result widget-shell contracts.

This foundation is not settings editing. It does not add settings controls, backend selector UI, SQLite path editor UI, provider auto-init controls, broad Apply/Cancel execution, settings save/config-write behavior beyond the guarded backend-only write, reset/delete actions, widget blueprint assets, maps, startup wiring, viewport attachment, timers, tick, polling, auto-refresh, provider/repository initialization, migrations, seed-copy behavior, or default config changes.

The draft validation, apply-preview, apply/cancel contract, and apply-result display paths remain caller-invoked. Validation, dry-run preview generation, apply/cancel contract generation, default apply request evaluation, validation display generation, apply-preview display generation, apply/cancel contract display generation, apply-result display generation, the SQLUISamples presenters/adapters, and the C++ widget shells do not create databases or directories, open databases for writing, run migrations, copy seeds, initialize providers/repositories, or delete files outside smoke-owned cleanup. The explicit backend-only production apply request is the only mutation in this area and writes just `Backend=<value>` to the selected settings file. `InMemory` remains the safe default, SQLite remains explicit opt-in, provider auto-init remains off by default, and no default config creates SQLite database files.

Future broader mutating apply/cancel work should reuse SQLUICore helper and policy surfaces. Widgets should capture user intent and display SQLUICore-provided validation/status/contract data; they should not know SQL, schema, migration ids, seed-copy policy, SQLite sidecar internals, direct deletion rules, or provider/repository lifecycle details. Widgets also should not write config directly, initialize providers/repositories, or delete files directly. Reset/delete UX should route through SQLUICore database management helper/policy surfaces. Any future actual apply/cancel implementation needs focused smoke coverage, and any startup, default map, config, viewport, provider lifecycle, or packaged runtime lifecycle change needs packaged validation.
...
This foundation is still not a product settings screen. It adds no broad apply/save/config-write behavior, settings editing controls, backend selector UI, SQLite path editor UI, provider auto-init controls, reset/delete UX, widget blueprint assets, maps, startup wiring, viewport attachment, timers, tick, polling, auto-refresh, provider/repository initialization, migrations, seed-copy behavior, or default config changes. Refresh/build/validation/preview/contract generation remains caller-invoked only.
...
The non-mutating apply/cancel contract UI foundation is complete as a checkpoint only. It closes the #105-#124 settings display chain before any broad settings apply/save/config-write behavior, settings controls, backend selector, SQLite path editor, provider auto-init control, reset/delete UX, widget blueprint asset, map, startup wiring, viewport attachment, polling, ticking, auto-refresh, provider/repository initialization, migration, seed-copy behavior, or default config change is added.

The checkpoint includes the #105 editing/reset UX plan, #106 validation-only draft model, #107 validation display rows, #108 draft validation adapter, #109 draft validation C++ UMG shell, #110 draft validation usage guide, #111 draft validation checkpoint, #112 dry-run apply preview, #113 apply-preview display rows, #114 apply-preview adapter, #115 apply-preview C++ UMG shell, #116 apply-preview usage guide, #117 apply-preview checkpoint, #118 non-mutating apply/cancel contract, #119 apply/cancel contract display rows, #120 editor launch validation checkpoint, #121 apply/cancel contract adapter, #122 apply/cancel contract C++ UMG shell, #123 apply/cancel contract usage guide, #124 final docs-only foundation checkpoint, #126 apply entrypoint, #127 apply-result display rows, the SQLUISamples apply-result sample adapter, the optional apply-result C++ UMG shell, and `-UsePersistenceSettingsDraftProbe` smoke coverage for the validation/apply-preview/apply-contract/apply-result model, display rows, sample adapters, widget-shell contracts, and backend-only production apply write.

The contract and default apply-request paths remain non-mutating. They report whether a future Apply would be available, why it may be blocked, that broad/default Apply execution is unavailable/not implemented, and what cancel/discard would mean as pure value state. The explicit backend-only production apply request is the first narrow exception: it can write only the backend value to the selected SQLUICore-owned settings file. These paths do not write SQLite path, provider auto-init, migration, seed, reset, or delete settings; they do not change live provider state, create databases, open databases for writing, run migrations, copy seed databases, initialize providers/repositories, delete files, change startup behavior, or make SQLite active just because a draft/contract/apply request was built. `InMemory` remains the safe default backend, SQLite remains opt-in, provider auto-init remains off by default, and no default config creates SQLite database files.

Future broader mutating apply/cancel work should use SQLUICore helper/policy surfaces and stay split into small PRs after the backend-only write: expand settings coverage only after validation and contract surfaces remain stable, and add UI controls only after SQLUICore behavior has focused smoke coverage. Widgets should remain presentation/intent-capture only: no direct SQL/schema/migration/seed/sidecar knowledge, no direct config writes, no direct provider/repository initialization, and no direct file deletion. Reset/delete behavior must route through SQLUICore database management helper/policy surfaces, and startup/config/default-map/provider lifecycle changes need packaged validation.
...
- `-UsePersistenceSettingsDraftProbe`: verifies the SQLUICore draft/validation model, dry-run apply preview, non-mutating apply/cancel contract, default apply entrypoint refusal, validation display rows, apply-preview display rows, apply/cancel contract display rows, apply-result display rows, the smoke-owned apply config target scaffold, the apply config target policy/resolver skeleton, the backend-only production apply write, explicit backend-only runtime settings readback, SQLUISamples validation/apply-preview/apply-contract/apply-result sample adapters, and optional validation/apply-preview/apply-contract/apply-result C++ UMG widget shell reflection contracts for default/current `InMemory`, unknown backend rejection/error display/preview-display/blocked-contract/contract-display/apply-result-display/apply-result-adapter/apply-result-widget error, SQLite draft/path display/preview-display/contract/contract-display/apply-result-display/apply-result-adapter/apply-result-widget without DB creation, empty SQLite path rejection/error display/preview-display/blocked-contract/contract-display error, provider auto-init pending validation/display/preview-display/contract/contract-display/apply-result-display/apply-result-adapter/apply-result-widget without policy mutation, deterministic validation/display/preview/preview-display/contract-display/apply-result-display/apply-result-adapter/apply-result-widget/contract/adapter output, default/runtime target unavailable, explicit smoke-owned target write capability, documented production strategy non-writable/no-writable-path resolution, selected production descriptor resolution for `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini`, guarded production target enablement request/no-request behavior, backend-only production writes that serialize only `Backend`, absent/valid/invalid backend-only readback without applying runtime settings, invalid/no-change/provider-auto-init refusal without mutation, no SQLite path serialization or readback, no provider auto-init serialization or readback, no DB creation, smoke-owned sidecar preservation, and cleanup.
...
- Implementing the user-facing persistence settings UI described in [`sqlui_persistence_settings_ux_design.md`](sqlui_persistence_settings_ux_design.md), [`sqlui_persistence_settings_editing_reset_plan.md`](sqlui_persistence_settings_editing_reset_plan.md), [`sqlui_persistence_status_umg_usage.md`](sqlui_persistence_status_umg_usage.md), [`sqlui_persistence_settings_draft_umg_usage.md`](sqlui_persistence_settings_draft_umg_usage.md), [`sqlui_persistence_settings_apply_preview_umg_usage.md`](sqlui_persistence_settings_apply_preview_umg_usage.md), [`sqlui_persistence_settings_apply_contract_umg_usage.md`](sqlui_persistence_settings_apply_contract_umg_usage.md), and [`sqlui_persistence_settings_apply_result_umg_usage.md`](sqlui_persistence_settings_apply_result_umg_usage.md). The first read-only SQLUICore status snapshot, display rows, sample presenter, panel adapter, C++ widget shell, panel contract, focused binding guide, docs-only mutating settings/reset plan, non-mutating draft validation model, dry-run apply-intent preview, non-mutating apply/cancel contract, non-mutating apply request skeleton, draft validation display rows, apply-preview display rows, apply/cancel contract display rows, apply-result display rows, explicit backend-only write, explicit backend-only readback, sample/dev-facing validation/apply-preview/apply-contract/apply-result adapters, validation/apply-preview/apply-contract/apply-result C++ widget shells, focused draft/apply-preview/apply-contract/apply-result binding guides, and docs-only actual apply implementation gate exist; widget blueprint assets, visual layout, broader mutating settings apply/save/config-write execution, startup consumption of `RuntimeSettings.ini`, backend selector UI, SQLite path editor UI, provider auto-init controls, reset UI, and product startup policy remain future work.
