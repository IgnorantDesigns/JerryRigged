...
This document plans the next SQLUI persistence settings UX phase after the completed read-only status foundation. PR #105 was a docs-only design and sequencing PR. The first implementation slices now add a non-mutating SQLUICore draft/pending settings model, validation-only behavior, UI-safe validation display rows, a dry-run apply-intent preview, a non-mutating apply/cancel contract, UI-safe apply-preview display rows, UI-safe apply/cancel contract display rows, a callable actual apply entrypoint skeleton that still returns unavailable/not implemented without mutation, UI-safe apply-result display rows, SQLUISamples sample/dev adapters for validation, apply-preview, apply/cancel contract, and apply-result display rows, optional C++ UMG widget shell contracts for future Blueprint binding, including the apply-result shell, a smoke-owned apply config target scaffold, the #134 apply config target policy/resolver skeleton, documented production target strategy resolution, and the #139 guarded production target enablement request/resolution; they still do not implement settings editing UI controls, backend selector controls, SQLite path editor controls, provider auto-init toggle controls, reset/delete behavior, settings save/apply execution, production/user config writes, startup wiring, widget blueprint assets, config, Build.cs changes, plugin descriptor changes, maps, Content assets, packaged outputs, or packaged runtime behavior.
...
The current recommendation is codified in SQLUICore policy as a documented strategy resolution: production/default Apply stays unavailable because no existing production write target is safe enough to enable implicitly. The guarded production target enablement request does not change that recommendation; it records an explicit enablement attempt and returns a blocked/non-writable result until a concrete target is selected. A future implementation PR must deliberately choose a SQLUICore-owned target, keep smoke-owned and real target paths separate, validate drafts before writes, refuse invalid drafts without mutation, preserve no-change/no-op behavior, prove committed config remains unchanged, prove no DB files are created by config Apply alone, prove no provider/repository lifecycle work runs, and add packaged validation if startup/config/default-map/provider lifecycle or packaged runtime behavior can be affected.

PR #139 proves the enablement request can be explicit and testable without enabling writes. It does not select a production target, produce a target file/path, apply settings, mutate config, initialize providers or repositories, create/open DBs for writing, run migrations, copy seeds, or delete files outside smoke-owned cleanup. The first real write PR must still name the exact storage surface, justify why it is safe, keep `DefaultEngine.ini`, committed defaults, user/global editor settings, implicit `Saved/Config`, and widget-owned writes out of scope, and preserve `InMemory` default, SQLite opt-in, and provider auto-init default-off behavior.
