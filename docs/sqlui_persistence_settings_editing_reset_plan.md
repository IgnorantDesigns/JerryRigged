...
PR #134 added a small apply config target policy/resolver skeleton layered above that scaffold. It makes the target decision explicit without enabling production Apply: default/runtime targets resolve unavailable and non-writable, explicit safe smoke-owned targets resolve smoke/test-only and writable for commandlet coverage, and a future project/user config target is represented as unavailable/not implemented until a later PR scopes the real target. PR #135 is a docs-only checkpoint for that policy/resolver skeleton; it adds no runtime code, scripts, config writes, smoke flags, UI controls, assets, maps, generated files, packaged outputs, database files, or behavior changes. The production/default `RequestPersistenceSettingsApply` path remains the existing unavailable/non-mutating apply entrypoint; it writes no config, changes no settings, and performs no lifecycle or database work. A follow-up policy-resolution slice now codifies the #136 documented production target strategy as `ResolveDocumentedProductionTargetStrategy()`: it resolves to the future project/user target kind, exposes no writable path, keeps `bCanWrite=false`, keeps production Apply disabled, and delegates the older future-target resolver through that same non-writable strategy.

The production config target resolution checkpoint is documentation-only and confirms what #137 proves: SQLUICore has a stable policy-resolution point for the documented future real target, but that target is still unavailable/non-writable, no real path is selected, and explicit smoke-owned targets remain the only write-capable path. It does not add production/user config writes, runtime settings application, UI controls, default config changes, provider/repository lifecycle behavior, DB creation, DB write-open, migrations, seed copy, or file deletion outside smoke-owned cleanup. The next write-enable phase must explicitly flip a real target from unavailable to writable, keep the resolver gate in the write path, validate drafts before writing, refuse invalid drafts without mutation, preserve no-change/no-op behavior, and prove repo `Config` / `Saved/Config` preservation plus smoke-owned versus real-target separation.
