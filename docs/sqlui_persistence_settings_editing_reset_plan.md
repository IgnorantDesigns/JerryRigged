...
This document plans the next SQLUI persistence settings UX phase after the completed read-only status foundation. PR #105 was a docs-only design and sequencing PR. The first implementation slices now add a SQLUICore draft/pending settings model, validation-only behavior, UI-safe validation display rows, a dry-run apply-intent preview, a non-mutating apply/cancel contract, UI-safe apply-preview display rows, UI-safe apply/cancel contract display rows, a callable apply entrypoint with default unavailable behavior plus a guarded backend-only production write, UI-safe apply-result display rows, SQLUISamples sample/dev adapters for validation, apply-preview, apply/cancel contract, and apply-result display rows, optional C++ UMG widget shell contracts for future Blueprint binding, including the apply-result shell, a smoke-owned apply config target scaffold, the #134 apply config target policy/resolver skeleton, documented production target strategy resolution, the #139 guarded production target enablement request/resolution, and the concrete production target design at `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini`; they still do not implement settings editing UI controls, backend selector controls, SQLite path editor controls, provider auto-init toggle controls, reset/delete behavior, broad settings save/apply execution, SQLite path/provider-auto-init production writes, startup wiring, widget blueprint assets, config, Build.cs changes, plugin descriptor changes, maps, Content assets, packaged outputs, or packaged runtime behavior.
...
This closes the display/adapter/widget-shell chain for apply results before broader config-write behavior begins. It is still not settings editing. It adds no broad apply/save/config-write behavior, backend selector UI, SQLite path editor UI, provider auto-init control, reset/delete UX, widget Blueprint asset, map, startup wiring, viewport attachment, polling, ticking, timer, auto-refresh, provider/repository initialization, migration, seed-copy behavior, or default config change. Refresh/build/validation/preview/contract/apply-result generation remains caller-invoked only.
...
PR #134 added a small apply config target policy/resolver skeleton layered above that scaffold. It makes the target decision explicit without enabling broad production Apply: default/runtime targets resolve unavailable and non-writable, explicit safe smoke-owned targets resolve smoke/test-only and writable for commandlet coverage, and the project/runtime config target is represented separately from unsafe committed config paths. PR #135 is a docs-only checkpoint for that policy/resolver skeleton; it adds no runtime code, scripts, config writes, smoke flags, UI controls, assets, maps, generated files, packaged outputs, database files, or behavior changes. The production/default `RequestPersistenceSettingsApply` path remains unavailable/non-mutating; it writes no config, changes no settings, and performs no lifecycle or database work. A follow-up policy-resolution slice codified the #136 documented production target strategy as `ResolveDocumentedProductionTargetStrategy()`: it resolves to the selected project/runtime target kind, exposes no broad writable path, keeps `bCanWrite=false`, and delegates the older future-target resolver through that same non-writable strategy. The selected `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` target is descriptor-backed for validation/display. A guarded backend-only request can now create/write that selected target, but it serializes only `Backend=<value>` and leaves SQLite path/provider auto-init/migration/seed/reset/delete settings out of scope.

The production config target resolution checkpoint confirms what #137 proves: SQLUICore has a stable policy-resolution point for the selected target, and the descriptor slice names `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` without making default/broad Apply writable. The guarded enablement request slice records explicit production target enablement intent. The backend-only write slice is the first intentionally narrow selected-target mutation: it requires the guarded backend-only request, validates the draft, writes only the backend value, preserves no-change/no-op behavior, rejects invalid/provider-auto-init/out-of-scope changes without mutation, and proves repo `Config` / `Saved/Config` preservation plus smoke-owned versus selected-target separation. This plan still does not add broad production/user config writes, runtime settings application, UI controls, default config changes, provider/repository lifecycle behavior, DB creation, DB write-open, migrations, seed copy, or file deletion outside smoke-owned/probe-created cleanup. The next broader write-enable phase must keep the resolver gate in the write path, validate drafts before writing, refuse invalid drafts without mutation, preserve no-change/no-op behavior, and extend smoke coverage for each newly writable setting.

## Backend-Only Apply Write Checkpoint

PR #143 establishes the first production-target write checkpoint and keeps it intentionally backend-only. The selected target is `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini`; it is created only by an explicit guarded backend-only apply request and contains only the selected backend value. The default Apply path, status panels, descriptor lookup, and enablement resolution do not create the file, and commandlet smoke cleanup removes the probe-created file and empty parent directory.

This checkpoint does not make a settings screen ready. It does not add backend selector controls, SQLite path editor controls, provider auto-init controls, reset/delete controls, widget Blueprint assets, maps, startup wiring, viewport attachment, timers, polling, provider/repository initialization, DB creation, DB write-open, migrations, seed copy, or destructive actions. It also does not write SQLite path, provider auto-init, migration, seed, schema-init, reset, delete, or lifecycle settings.

The next expansion order remains conservative: keep the backend-only write and explicit backend-only readback as the current checkpoint; keep readback caller-invoked and out of startup; add any broader settings merge/preview behavior as caller-invoked reporting first; add startup consumption separately only after that behavior is smoke-tested; then add provider auto-init write in a separate policy PR; then add SQLite path write in a path-safety PR; then add backend selector UI after SQLUICore behavior is fully tested. Any PR that makes startup, packaged runtime, provider lifecycle, default maps, or runtime config loading consume this file needs packaged validation before claiming production readiness.

The apply entrypoint and apply-result display path can report validation-blocked, no-change, preview-only, unavailable/not-implemented, and guarded backend-only result states. Default requests still do not write config, apply settings, change provider auto-init policy, initialize providers/repositories, create directories or database files, open databases for writing, run migrations, copy seed databases, reset/delete data, or change startup behavior. The guarded backend-only production request may create/write the selected settings file, but it writes only the backend value.

## Backend-Only Runtime Settings Readback Checkpoint

PR #145 added a backend-only readback helper for the selected `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` target. This is a parser/checkpoint, not runtime settings consumption. The helper is called explicitly, reads only the `Backend` value, treats an absent file as no runtime override, returns a parsed backend for valid content, and reports missing or unknown backend values as errors without repairing, deleting, rewriting, or silently falling back.

The readback helper does not read SQLite database path, provider auto-init, migration, seed, schema-init, reset/delete, startup, lifecycle, or UI-control settings. It does not apply the parsed backend to `USQLUILayoutRepositoryRuntimeSettings`, initialize or reinitialize providers/repositories, create database files, open SQLite for writing, run migrations, copy seeds, delete files, or change startup behavior. `-UsePersistenceSettingsDraftProbe` covers absent-file readback, valid readback after the backend-only write, unknown/missing backend rejection, repo `Config` / `Saved\Config` preservation, no DB/lifecycle side effects, and cleanup of probe-created `RuntimeSettings.ini` artifacts.
...
Future mutating work should stay split into small PRs: first a real write target policy decision that turns the currently unavailable project/user target into a deliberately chosen and validated target, then a narrow backend/provider-auto-init config-write slice only when safe, then validation/no-op/failure paths, then smoke-owned versus real-target separation checks, and then UI controls only after SQLUICore behavior is fully tested. Any such path needs smoke coverage, config diff/snapshot checks, cleanup/restore for smoke-owned artifacts, no-op behavior for unchanged drafts, lifecycle/database-work separation from config writes, and packaged validation if startup behavior, default maps, config wiring, provider lifecycle, viewport flow, packaged runtime behavior, or committed/default runtime config changes.

## Production Config Target Strategy Gate

The production config target decision is now documented in [`sqlui_persistence_config_target_strategy.md`](sqlui_persistence_config_target_strategy.md). This gate exists before broad user/runtime Apply writes are enabled. It inventories committed project defaults, generated `Saved/Config`, user/global editor settings, the existing `USQLUILayoutRepositoryRuntimeSettings` config-backed surface, possible SQLUICore-owned `Saved/SQLUI` runtime files, plugin-managed `Saved/SQLUI` files, explicit smoke-owned targets, the guarded backend-only selected target write, and the option to keep broad production Apply unavailable.

The current recommendation is codified in SQLUICore policy as a documented strategy resolution: production/default Apply stays unavailable for broad writes, while the concrete target design is `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini`, a SQLUICore-owned settings file under project `Saved` storage. The guarded production target enablement request does not change runtime behavior by itself; the explicit backend-only apply request is the only implemented selected-target write and serializes just `Backend=<value>`. Future implementation PRs must keep smoke-owned and selected target paths separate, validate drafts before writes, refuse invalid drafts without mutation, preserve no-change/no-op behavior, prove committed config remains unchanged, prove no DB files are created by config Apply alone, prove no provider/repository lifecycle work runs, and add packaged validation if startup/config/default-map/provider lifecycle or packaged runtime behavior can be affected.

PR #139 proves the enablement request can be explicit and testable without enabling broad writes. The backend-only write slice proves the selected storage surface can be created/written narrowly, but it does not apply settings to live runtime state, initialize providers or repositories, create/open DBs for writing, run migrations, copy seeds, or delete files outside smoke-owned/probe-created cleanup. Future write PRs must justify each additional setting in code and smoke coverage, keep `DefaultEngine.ini`, committed defaults, user/global editor settings, implicit `Saved/Config`, and widget-owned writes out of scope, and preserve `InMemory` default, SQLite opt-in, and provider auto-init default-off behavior.

Unsafe targets remain rejected: runtime Apply must not write `DefaultEngine.ini`, mutate committed plugin/project defaults, write user/global editor settings unexpectedly, use widget-owned file/config writes, infer `Saved/Config` as a target without separate validation, silently change startup behavior or provider lifecycle, or create SQLite database files as a side effect. The first real write slice should be narrow, preferably backend choice only; SQLite path writing should remain a later path-specific PR.
...
The first SQLUICore actual apply entrypoint exists as `RequestPersistenceSettingsApply`. It accepts `FSQLUIPersistenceSettingsApplyRequest`, evaluates the existing apply contract, returns `FSQLUIPersistenceSettingsApplyResult`, and keeps default execution unavailable/not implemented. It can report validation-blocked, no-change, preview-only, failed, and succeeded states. The only implemented success path is the explicit backend-only production request; default requests still keep `bSucceeded=false`, `bActualApplyImplemented=false`, and all side-effect flags false.

Broad apply/save/config-write behavior has not been implemented yet. The current `PreviewPersistenceSettingsDraftApply`, `BuildPersistenceSettingsApplyContract`, and `BuildPersistenceSettingsCancelPreview` helpers remain non-mutating report/preview paths. `RequestPersistenceSettingsApply` remains non-mutating for default requests and can only write the backend value for the explicit guarded backend-only production request. These helpers do not save SQLite paths, write provider auto-init policy, initialize provider or repository state, create database files, open databases for writing, run migrations, copy seed databases, delete files, or change startup behavior.
...
2. Add config write for a narrow subset only when the selected `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` target is implemented safely, starting with backend-only settings before provider auto-init or SQLite path persistence.
...
The first apply/cancel phase code slices remain mostly non-mutating. `USQLUIPersistenceSettingsDraftLibrary::PreviewPersistenceSettingsDraftApply` evaluates a validated draft and reports whether a future Apply would have changes, whether backend/SQLite/provider-auto-init policy would change, and whether restart/reopen/reinitialize messaging should be shown. `BuildPersistenceSettingsApplyContract` wraps that preview with an explicit availability/readiness contract and reports that broad Apply execution is not implemented. `RequestPersistenceSettingsApply` returns unavailable/not implemented, validation-blocked, no-change, preview-only, failed, or succeeded result data; success is limited to the explicit guarded backend-only production request. `BuildPersistenceSettingsCancelPreview` reports whether cancel would discard pending values and returns the draft value that would result, without touching live settings.

The preview, contract, apply request, apply-result display rows, optional SQLUISamples apply-preview presenter, optional SQLUISamples apply/cancel contract presenter, optional SQLUISamples apply-result presenter, optional apply-preview widget shell, optional apply/cancel contract widget shell, and optional apply-result widget shell deliberately use clear wording such as "would change," "not implemented," "not applied," "not saved," "config written/not written," and "database files not created." They do not apply live provider settings, save SQLite paths, create database files, open databases for writing, run migrations, copy seed databases, initialize providers/repositories, reset databases, delete files, add settings controls, change defaults, or change startup behavior. `-UsePersistenceSettingsDraftProbe` covers the preview, contract, apply request, apply-result display rows, apply-result adapter, and apply-result widget shell contract alongside draft validation/display rows, apply-preview display rows/adapters, apply/cancel contract display rows/adapter, and widget shell contracts, and verifies default/current no-change previews, backend/SQLite/provider-auto-init change previews, invalid backend/path rejection, cancel/discard value preview, deterministic output, sidecar preservation, config preservation, backend-only selected-target writing, and smoke-owned/probe-created cleanup.
...
11. Add UI-safe apply-result display rows for default unavailable apply behavior and guarded backend-only apply behavior. Complete as a SQLUICore-only display formatter with smoke coverage through `-UsePersistenceSettingsDraftProbe`.
