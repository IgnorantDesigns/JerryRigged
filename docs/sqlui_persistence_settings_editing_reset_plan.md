...
This document plans the next SQLUI persistence settings UX phase after the completed read-only status foundation. PR #105 was a docs-only design and sequencing PR. The first implementation slices now add a non-mutating SQLUICore draft/pending settings model, validation-only behavior, UI-safe validation display rows, a dry-run apply-intent preview, a non-mutating apply/cancel contract, UI-safe apply-preview display rows, UI-safe apply/cancel contract display rows, a callable actual apply entrypoint skeleton that still returns unavailable/not implemented without mutation, UI-safe apply-result display rows, SQLUISamples sample/dev adapters for validation, apply-preview, apply/cancel contract, and apply-result display rows, optional C++ UMG widget shell contracts for future Blueprint binding, including the apply-result shell, a smoke-owned apply config target scaffold, the #134 apply config target policy/resolver skeleton, documented production target strategy resolution, the #139 guarded production target enablement request/resolution, and a docs-only concrete production target design for `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini`; they still do not implement settings editing UI controls, backend selector controls, SQLite path editor controls, provider auto-init toggle controls, reset/delete behavior, settings save/apply execution, production/user config writes, startup wiring, widget blueprint assets, config, Build.cs changes, plugin descriptor changes, maps, Content assets, packaged outputs, or packaged runtime behavior.
...
PR #134 added a small apply config target policy/resolver skeleton layered above that scaffold. It makes the target decision explicit without enabling production Apply: default/runtime targets resolve unavailable and non-writable, explicit safe smoke-owned targets resolve smoke/test-only and writable for commandlet coverage, and a future project/user config target is represented as unavailable/not implemented until a later PR scopes the real target. PR #135 is a docs-only checkpoint for that policy/resolver skeleton; it adds no runtime code, scripts, config writes, smoke flags, UI controls, assets, maps, generated files, packaged outputs, database files, or behavior changes. The production/default `RequestPersistenceSettingsApply` path remains the existing unavailable/non-mutating apply entrypoint; it writes no config, changes no settings, and performs no lifecycle or database work. A follow-up policy-resolution slice now codifies the #136 documented production target strategy as `ResolveDocumentedProductionTargetStrategy()`: it resolves to the future project/user target kind, exposes no writable path, keeps `bCanWrite=false`, keeps production Apply disabled, and delegates the older future-target resolver through that same non-writable strategy. A guarded enablement request resolver now records when production target enablement is explicitly requested, but because the selected `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini` target is design-only and not implemented, it still returns blocked/non-writable/pathless policy output instead of enabling writes.

The production config target resolution checkpoint is documentation-only and confirms what #137 proves: SQLUICore has a stable policy-resolution point for the documented future real target, but that target is still unavailable/non-writable, no write-enabled path is selected in code, and explicit smoke-owned targets remain the only write-capable path. The guarded enablement request slice adds a policy-only way to request production target enablement and prove that the request is recorded, blocked, non-writable, pathless, deterministic, and side-effect free while the target remains unimplemented. The concrete design now selects a SQLUICore-owned target at `Saved/SQLUI/PersistenceSettings/RuntimeSettings.ini`, but this plan still does not add production/user config writes, runtime settings application, UI controls, default config changes, provider/repository lifecycle behavior, DB creation, DB write-open, migrations, seed copy, or file deletion outside smoke-owned cleanup. The next real write-enable phase must implement the selected target, then explicitly flip that selected target from unavailable to writable through policy, keep the resolver gate in the write path, validate drafts before writing, refuse invalid drafts without mutation, preserve no-change/no-op behavior, and prove repo `Config` / `Saved/Config` preservation plus smoke-owned versus real-target separation.
