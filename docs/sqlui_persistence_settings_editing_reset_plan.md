...
SQLUICore now has a small apply config target policy/resolver skeleton layered above that scaffold. It makes the target decision explicit without enabling production Apply: default/runtime targets resolve unavailable and non-writable, explicit safe smoke-owned targets resolve smoke/test-only and writable for commandlet coverage, and a future project/user config target is represented as unavailable/not implemented until a later PR scopes the real target. The production/default `RequestPersistenceSettingsApply` path remains the existing unavailable/non-mutating apply entrypoint; it writes no config, changes no settings, and performs no lifecycle or database work.
